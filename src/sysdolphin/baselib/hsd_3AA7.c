#include "hsd_3AA7.h"

#include "hsd_3A94.h"
#include "hsd_3B2B.h"
#include "hsd_3B2E.h"

#include <dolphin/card.h>
#include <dolphin/os.h>

#define CARD_QUEUE_CMD(cmd) fn_803AC168((cmd))
#define CARD_WRITE_BLOCK(state, block_idx, file_id, seq_num, payload,         \
                         payload_len, file_idx)                               \
    fn_803ACFC0((state), (block_idx), (file_id), (seq_num), (payload),        \
                (payload_len), (file_idx))

/* 4D7980 */ extern volatile s32 hsd_804D7980;
/* 4D7984 */ extern volatile s32 hsd_804D7984;
/* 4D7988 */ extern s32 hsd_804D7988;
/* 4D798C */ extern s32 hsd_804D798C;
/* 4D7998 */ extern s32 hsd_804D7998;
/* 4D799C */ extern s32 hsd_804D799C;

typedef struct CardBufEntry {
    s32 x0, x4, x8, xC;
    s32 x10;
    s32 x14, x18, x1C, x20;
} CardBufEntry;

typedef struct CardCmd {
    /* 0x00 */ s32 type;
    /* 0x04 */ CardState* state;
    /* 0x08 */ s32 x8;
    /* 0x0C */ s32 xC;
    /* 0x10 */ s32 x10;
    /* 0x14 */ s32 x14;
    /* 0x18 */ void* x18;
    /* 0x1C */ s32 x1C;
    /* 0x20 */ s32 x20;
} CardCmd;

typedef struct HsdCmdEntry {
    s32 type;
    s32 f1;
    s32 f2;
    s32 f3;
    s32 f4;
    s32 f5;
} HsdCmdEntry;

typedef struct CardContext {
    /* 0x0000 */ s32 x0;
    /* 0x0004 */ CardState* x4;
    /* 0x0008 */ void (*x8)(s32, s32);
    /* 0x000C */ s32 xC;
    /* 0x0010 */ CardCmd x10[128];
    /* 0x1210 */ HsdCmdEntry x1210[32];
} CardContext;

typedef struct CardQueueEntry {
    /* 0x00 */ s32 x0;
    /* 0x04 */ s32 x4;
    /* 0x08 */ s32 x8;
    /* 0x0C */ s32 xC;
    /* 0x10 */ s32 x10;
    /* 0x14 */ void (*x14)(s32, s32);
} CardQueueEntry;

#define CMD_QUEUE(base) ((HsdCmdEntry*) ((base) + 0x1210))

s32 fn_803AA790(void)
{
    CardQueueEntry* entry;
    s32 result;
    s32 arg0;

    entry = &((CardQueueEntry*) &hsd_804D2348)[hsd_804D7990];
    arg0 = entry->x4;
    hsd_804D7990 = (hsd_804D7990 + 1) % 32;

    switch (entry->x0) {
    case 1:
        result =
            fn_803ADF90((struct CardState*) entry->x4, entry->x8,
                        (u8*) entry->xC, 1, (void (*)(s32, s32)) entry->x14);
        if (result < 0) {
            if (entry->x14 != NULL) {
                entry->x14(entry->x8, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 2:
        switch (((s32*) (arg0 + 0x28))[entry->x8]) {
        case 0:
            result = fn_803AE7F8((struct CardState*) entry->x4, entry->x8,
                                 (u8*) entry->xC, 1,
                                 (void (*)(s32, s32)) entry->x14);
            break;
        case 1:
        case 2:
            result =
                fn_803AF3F0((CardState*) entry->x4, entry->x8, (u8*) entry->xC,
                            1, (void (*)(s32, s32)) entry->x14);
            break;
        case 3:
            result =
                fn_803B0120((CardState*) entry->x4, entry->x8, (u8*) entry->xC,
                            1, (void (*)(s32, s32)) entry->x14);
            break;
        default:
            result = -0x101;
            break;
        }
        if (result < 0) {
            if (entry->x14 != NULL) {
                entry->x14(entry->x8, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 3:
        result = fn_803B1F78((CardState*) entry->x4, entry->x8, entry->xC,
                             entry->x10, (s32) entry->x14);
        if (result < 0) {
            if (entry->x14 != NULL) {
                entry->x14(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 4:
        result =
            fn_803B21E8(entry->x4, entry->xC, entry->x10, (s32) entry->x14);
        if (result < 0) {
            if (entry->x14 != NULL) {
                entry->x14(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 5:
        result = fn_803ADE4C(arg0, entry->x8, (s32) entry->x14);
        if (result < 0) {
            if (entry->x14 != NULL) {
                entry->x14(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    case 6:
        result = fn_803B26CC((CardState*) arg0, entry->x8, entry->xC,
                             entry->x10, (void (*)(s32, s32)) entry->x14);
        if (result < 0) {
            if (entry->x14 != NULL) {
                entry->x14(0, result);
            }
        }
        entry->x0 = 0;
        return result;
    default:
        return -0x101;
    }
}

static inline s32 retryCardFastOpen(s32 chan, s32 file_no,
                                    CARDFileInfo* file_info)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDFastOpen(chan, file_no, file_info);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

static inline s32 retryCardReadAsync(CARDFileInfo* file_info, void* buffer,
                                     s32 length, s32 offset,
                                     CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDReadAsync(file_info, buffer, length, offset, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

static inline s32 retryCardWriteAsync(CARDFileInfo* file_info, void* buffer,
                                      s32 length, s32 offset,
                                      CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDWriteAsync(file_info, buffer, length, offset, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

static inline s32 retryCardClose(CARDFileInfo* file_info)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDClose(file_info);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

static inline s32 retryCardCreateAsync(s32 chan, char* filename, s32 size,
                                       CARDFileInfo* file_info,
                                       CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDCreateAsync(chan, filename, size, file_info, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

static inline s32 retryCardGetStatus(s32 chan, s32 file_no, CARDStat* stat)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDGetStatus(chan, file_no, stat);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

static inline s32 retryCardSetStatusAsync(s32 chan, s32 file_no,
                                          CARDStat* stat,
                                          CARDCallback callback)
{
    s32 i;
    s32 result;

    i = 0;
    do {
        result = CARDSetStatusAsync(chan, file_no, stat, callback);
        if (result != -1) {
            break;
        }
        i++;
    } while (i < 10);

    return result;
}

#define CMD_TYPE cmd[0]
#define CMD_STATE ((CardState*) cmd[1])
#define CMD_X8 cmd[2]
#define CMD_XC cmd[3]
#define CMD_X10 cmd[4]
#define CMD_X14 cmd[5]
#define CMD_X18 ((void*) cmd[6])
#define CMD_X1C cmd[7]
#define CMD_X20 cmd[8]

static inline int setupCardIcons(s32* cmd)
{
    u32 bit;
    s32 k;
    u8 speed;

    k = (bit = 0);
    for (; k < 8 && CMD_STATE->icon_speed[k] != 0;) {
        CMD_STATE->stat.iconFormat =
            (CMD_STATE->stat.iconFormat & ~(CARD_STAT_ICON_MASK << bit)) |
            (CMD_STATE->icon_format[k] << bit);
        speed = CMD_STATE->icon_speed[k++];
        {
            u16* iconSpeed = &CMD_STATE->stat.iconSpeed;
            *iconSpeed =
                (speed << bit) | (*iconSpeed & ~(CARD_STAT_SPEED_MASK << bit));
        }
        bit += 2;
    }
    return k;
}

static inline void unpackCardStat(s32* cmd, CARDStat* stat)
{
    s32 k;

    CMD_STATE->x3B0 = stat->bannerFormat & 3;
    for (k = 0; k < 8; k++) {
        CMD_STATE->icon_format[k] =
            (stat->iconFormat >> (2 * k)) & CARD_STAT_ICON_MASK;
        CMD_STATE->icon_speed[k] =
            (stat->iconSpeed >> (2 * k)) & CARD_STAT_SPEED_MASK;
    }
    CMD_STATE->x24 = hsd_803AC340(&CMD_STATE->x3B0);
    {
        u32 used = CMD_STATE->x24 + CMD_STATE->x8;
        used += 0x2F;
        CMD_STATE->x460 = stat->length / CMD_STATE->x8 - used / CMD_STATE->x8;
    }
}

void hsd_803AAA48(void)
{
    s32 r;
    BOOL intr2;

    while (1) {
        CardContext* ctx = (CardContext*) hsd_804D1138;
        CardState** state = &ctx->x4;
        s32* cmd;
        s32 type;
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return;
        }

        if (hsd_804D7988 < 0) {
            while (((CardBufEntry*) hsd_804D1138)[hsd_804D7980].x10 != 0) {
                ((CardBufEntry*) hsd_804D1138)[hsd_804D7980].x10 = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            }
        }

        cmd = (s32*) &((CardBufEntry*) ctx)[hsd_804D7980];
        type = *(cmd += 4);

        switch ((u32) type) {
        case 0:
            if (ctx->x0 != 0) {
                if (ctx->x0 == 3) {
                    s32 file = ctx->xC;
                    s32 blocks_before = fn_803AC6B8(*state, file);
                    s32 file_blocks = fn_803AC634(*state, file);
                    s32 total = fn_803AC7DC(*state);
                    s32 map[64];
                    s32 j;
                    for (j = 0; j < file_blocks; j++) {
                        map[j] = -1;
                    }
                    for (j = 1; j <= total; j++) {
                        s32 fidx = (*state)->x170[j];
                        if (fidx >= 0) {
                            s32 logical = fidx - blocks_before;
                            if (logical >= 0 && logical < file_blocks) {
                                if (map[logical] < 0) {
                                    map[logical] = j;
                                } else if (fn_803ACB74(
                                               (*state)->x270[map[logical]],
                                               (*state)->x270[j]) < 0)
                                {
                                    (*state)->x170[map[logical]] *= -1;
                                    map[logical] = j;
                                } else {
                                    (*state)->x170[j] *= -1;
                                }
                            }
                        }
                    }
                }
                if (ctx->x8 != 0) {
                    ctx->x8(ctx->xC, hsd_804D7988);
                }
                ctx->x0 = 0;
            }
            hsd_804D7988 = 0;
            hsd_804D799C = 2;
            if (hsd_804D7990 == hsd_804D7994 &&
                CMD_QUEUE((u8*) ctx)[hsd_804D7990].type == 0)
            {
                return;
            }
            if (fn_803AA790() < 0) {
                return;
            }
            hsd_804D799C = 0;
            continue;
        case 5:
            if (hsd_804D7988 == 2) {
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            }
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                   CMD_STATE->x8, CMD_X1C,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 11:
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
            if (CMD_X8 == 0) {
                memset(CMD_STATE->digest, 0, 0x30);
            }
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                   CMD_STATE->x8, CMD_X8 * CMD_STATE->x8,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 10:
            if (hsd_804D7988 == 2) {
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            }
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
            if (CMD_X8 == 0) {
                memset(CMD_STATE->digest, 0, 0x30);
            }
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                   CMD_STATE->x8, CMD_X8 * CMD_STATE->x8,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 6:
            if (hsd_804D7988 == 2) {
                hsd_804D7988 = 0;
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            } else {
                hsd_804D7988 = 1;
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                continue;
            }
        case 2:
            if (hsd_804D7988 != 1) {
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardReadAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                       CMD_STATE->x8, CMD_X1C,
                                       (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 15:
            if (hsd_804D7988 != 1) {
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardReadAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                       CMD_STATE->x8, CMD_X1C,
                                       (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 4:
            if (hsd_804D7988 != 1) {
                memset(CMD_X18, 0, CMD_X20);
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 1:
            if (hsd_804D7988 != 1) {
                s32 hdr_offset;
                s32 rem;
                signed int size;
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                if (CMD_X10 > 0) {
                    hdr_offset = 0;
                } else {
                    hdr_offset = (CMD_STATE->x24 + 0x30) % CMD_STATE->x8;
                }
                size = CMD_X20;
                if (size > 0 && CMD_X18 != NULL) {
                    memcpy(&CMD_STATE->x0[hdr_offset + 0x20], CMD_X18, size);
                }
                rem = (CMD_STATE->x8 - hdr_offset) - size - 0x20;
                if (rem != 0) {
                    memset(&CMD_STATE->x0[hdr_offset + size + 0x20], 0, rem);
                }
                memset(CMD_STATE->x0 + hdr_offset, 0, 0x20);
                CMD_STATE->x0[hdr_offset + 0x10] = (u8) (CMD_X10 >> 8);
                CMD_STATE->x0[hdr_offset + 0x11] = (u8) CMD_X10;
                CMD_STATE->x0[hdr_offset + 0x12] = (u8) CMD_X14;
                fn_803AC3F8(CMD_STATE, &CMD_STATE->x0[hdr_offset + 0x13],
                            CMD_X8);
                hsd_803B2FA0(CMD_STATE->x0 + hdr_offset,
                             CMD_STATE->x8 - hdr_offset);
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardWriteAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                        CMD_STATE->x8, CMD_X1C,
                                        (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 16:
            if (hsd_804D7988 != 1) {
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                hsd_803B2FA0(CMD_STATE->x0, CMD_STATE->x8);
                hsd_804D798C = CARDGetXferredBytes(CMD_STATE->x4);
                intr2 = OSDisableInterrupts();
                r = retryCardWriteAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                        CMD_STATE->x8, CMD_X1C,
                                        (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 7:
            intr2 = OSDisableInterrupts();
            r = retryCardCreateAsync(CMD_STATE->x4, (char*) CMD_X8, CMD_XC,
                                     &CMD_STATE->file_info,
                                     (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                continue;
            }
            return;
        case 8:
            if (hsd_804D7988 != 1) {
                int k;
                r = retryCardGetStatus(CMD_STATE->x4, CMD_STATE->x20,
                                       &CMD_STATE->stat);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                CMD_STATE->stat.commentAddr = 0;
                CMD_STATE->stat.iconAddr = 0x40;
                CMD_STATE->stat.bannerFormat =
                    (CMD_STATE->stat.bannerFormat & ~3) | CMD_STATE->x3B0;
                k = setupCardIcons(cmd);
                for (; k < 8; k++) {
                    CMD_STATE->stat.iconFormat =
                        CMD_STATE->stat.iconFormat &
                        ~(CARD_STAT_ICON_MASK << (2 * k));
                    CMD_STATE->stat.iconSpeed =
                        CMD_STATE->stat.iconSpeed &
                        ~(CARD_STAT_SPEED_MASK << (2 * k));
                }
                intr2 = OSDisableInterrupts();
                r = retryCardSetStatusAsync(CMD_STATE->x4, CMD_STATE->x20,
                                            &CMD_STATE->stat,
                                            (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 9:
            if (hsd_804D7988 != 1) {
                s32 icon_size;
                s32 pos;
                r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                      &CMD_STATE->file_info);
                if (r < 0) {
                    hsd_804D7988 = r;
                    continue;
                }
                switch (CMD_STATE->x3B0) {
                case 2:
                    icon_size = 0x1800;
                    break;
                case 1:
                    icon_size = 0xE00;
                    break;
                default:
                    icon_size = 0;
                    break;
                }
                if (CMD_X8 == 0) {
                    memcpy(CMD_STATE->x0, CMD_STATE->x370, 0x40);
                    pos = 0x40;
                    if (icon_size > 0) {
                        memcpy(CMD_STATE->x0 + 0x40, (void*) CMD_XC,
                               icon_size);
                        pos = icon_size + 0x40;
                    }
                    memset(CMD_STATE->digest, 0, 0x30);
                    if (CMD_STATE->x24 > CMD_STATE->x8) {
                        memcpy(&CMD_STATE->x0[pos], (void*) CMD_X10,
                               CMD_STATE->x8 - pos);
                        hsd_803B2B20(CMD_STATE->x0, CMD_STATE->x8,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                    } else {
                        memcpy(&CMD_STATE->x0[pos], (void*) CMD_X10,
                               CMD_STATE->x24 - pos);
                        hsd_803B2B20(CMD_STATE->x0, CMD_STATE->x24,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                        memcpy(&CMD_STATE->x0[CMD_STATE->x24],
                               CMD_STATE->digest, 0x30);
                    }
                } else {
                    u32 remaining = CMD_STATE->x24 - CMD_STATE->x8 * CMD_X8;
                    s32 data_off = (CMD_STATE->x8 * CMD_X8 - 0x40) - icon_size;
                    if (remaining > CMD_STATE->x8) {
                        memcpy(CMD_STATE->x0, (u8*) CMD_X10 + data_off,
                               CMD_STATE->x8);
                        hsd_803B2B20(CMD_STATE->x0, CMD_STATE->x8,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                    } else {
                        memcpy(CMD_STATE->x0, (u8*) CMD_X10 + data_off,
                               remaining);
                        hsd_803B2B20(CMD_STATE->x0, remaining,
                                     &CMD_STATE->digest[CMD_X8 * 0x10]);
                        memcpy(&CMD_STATE->x0[remaining], CMD_STATE->digest,
                               0x30);
                    }
                }
                intr2 = OSDisableInterrupts();
                r = retryCardWriteAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                        CMD_STATE->x8, CMD_STATE->x8 * CMD_X8,
                                        (void (*)(s32, s32)) hsd_803A949C);
                hsd_804D799C = 1;
                OSRestoreInterrupts(intr2);
                if (r < 0) {
                    hsd_804D799C = 0;
                    retryCardClose(&CMD_STATE->file_info);
                    hsd_804D7988 = r;
                    continue;
                }
                return;
            }
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        case 12: {
            CARDStat stat;
            CMD_STATE->x20 = CMD_X8;
            r = retryCardGetStatus(CMD_STATE->x4, CMD_STATE->x20, &stat);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
            } else {
                unpackCardStat(cmd, &stat);
                if (stat.iconAddr != 0x40) {
                    hsd_804D7988 = -0x106;
                    hsd_804D799C = 0;
                } else {
                    CMD_TYPE = 0;
                    hsd_804D7980 = (hsd_804D7980 + 1) % 128;
                }
            }
            continue;
        }
        case 13:
            r = retryCardFastOpen(CMD_STATE->x4, CMD_STATE->x20,
                                  &CMD_STATE->file_info);
            if (r < 0) {
                hsd_804D7988 = r;
                continue;
            }
            intr2 = OSDisableInterrupts();
            r = retryCardReadAsync(&CMD_STATE->file_info, CMD_STATE->x0,
                                   CMD_STATE->x8, CMD_X1C,
                                   (void (*)(s32, s32)) hsd_803A949C);
            hsd_804D799C = 1;
            OSRestoreInterrupts(intr2);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                retryCardClose(&CMD_STATE->file_info);
                continue;
            }
            return;
        case 14:
            r = fn_803AD16C(CMD_STATE);
            if (r < 0) {
                hsd_804D7988 = r;
                hsd_804D799C = 0;
            } else {
                CMD_TYPE = 0;
                hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            }
            continue;
        case 17: {
            s32 blk;

            fn_803AC2D4();
            r = fn_803ACF30(CMD_STATE, 0, 0, 0);
            if (r < 0) {
                fn_803AC2E0();
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                continue;
            }
            for (blk = 0; blk <= CMD_STATE->x460; blk++) {
                r = fn_803AC258(CMD_STATE, blk);
                if (r < 0) {
                    fn_803AC2E0();
                    hsd_804D7988 = r;
                    hsd_804D799C = 0;
                    goto next;
                }
            }
            r = fn_803AC2A4(CMD_STATE);
            if (r < 0) {
                fn_803AC2E0();
                hsd_804D7988 = r;
                hsd_804D799C = 0;
                continue;
            }
            fn_803AC334();
            CMD_TYPE = 0;
            hsd_804D7980 = (hsd_804D7980 + 1) % 128;
            continue;
        }
        default:
            return;
        }
    next:
        continue;
    }
}

#undef CMD_TYPE
#undef CMD_STATE
#undef CMD_X8
#undef CMD_XC
#undef CMD_X10
#undef CMD_X14
#undef CMD_X18
#undef CMD_X1C
#undef CMD_X20

s32 fn_803AC168(s32* cmd_buf)
{
    BOOL state;
    s32 mode;
    s32 read_idx;
    s32 old_entry;

    state = OSDisableInterrupts();
    read_idx = hsd_804D7980;
    mode = hsd_804D799C;
    old_entry = (s32) hsd_804D1148[hsd_804D7980][0];
    OSRestoreInterrupts(state);

    if (mode != 2) {
        if (hsd_804D7984 == read_idx) {
            if (mode != 0 || hsd_804D7984 != read_idx || old_entry != 0) {
                return -265;
            }
        }
    }

    {
        s32 idx = hsd_804D7984;
        hsd_804D7984 = (hsd_804D7984 + 1) % 128;
        memcpy((u8*) hsd_804D1148[idx], cmd_buf, 36);
    }

    if (mode == 2) {
        hsd_804D799C = 0;
    }
    return 0;
}

s32 fn_803AC258(CardState* card_state, s32 block_idx)
{
    s32 buf[9];
    buf[0] = 13;
    buf[1] = (s32) card_state;
    buf[4] = block_idx;
    buf[5] = 0;
    buf[6] = 0;
    buf[8] = 0;
    buf[7] = fn_803ACBE8(card_state, block_idx);
    return fn_803AC168(buf);
}

s32 fn_803AC2A4(CardState* card_state)
{
    s32 buf[9];
    buf[0] = 14;
    buf[1] = (s32) card_state;
    return fn_803AC168(buf);
}

void fn_803AC2D4(void)
{
    hsd_804D7998 = hsd_804D7984;
}

void fn_803AC2E0(void)
{
    s32 saved;

    if (hsd_804D7998 < 0) {
        return;
    }
    saved = hsd_804D7998;
    while (saved != hsd_804D7984) {
        hsd_804D1148[saved][0] = 0;
        saved = (saved + 1) % 128;
    }
    hsd_804D7984 = hsd_804D7998;
}

void fn_803AC334(void)
{
    hsd_804D7998 = -1;
}

/// @todo Currently 99.50% match - return expression register allocation (add
/// dest r6 vs r3)
int hsd_803AC340(void* header)
{
    u8* data = header;
    int base_size;
    int extra_size;
    int has_small;
    int i;

    switch (data[0]) {
    case 2:
        base_size = 0x1800;
        break;
    case 1:
        base_size = 0xE00;
        break;
    default:
        base_size = 0;
        break;
    }

    has_small = 0;
    extra_size = 0;
    i = 0;
    while (i < 8 && data[i + 0xA] != 0) {
        switch (data[i + 0x2]) {
        case 2:
            extra_size += 0x800;
            break;
        case 1:
            has_small = 1;
            extra_size += 0x400;
            break;
        }
        i++;
    }

    if (has_small != 0) {
        extra_size += 0x200;
    }

    extra_size = base_size + extra_size;
    return extra_size + 0x40;
}

void hsd_803AC3E0(struct CardState* file_desc, int file_idx, int file_size,
                  int file_flags, int data_size)
{
    file_desc->x28[file_idx] = file_flags;
    file_desc->x4C[file_idx] = file_size;
    file_desc->x70[file_idx].size = data_size;
}

void fn_803AC3F8(void* arg0, u8* data, s32 file_idx)
{
    struct CardState* file_desc = arg0;
    s32 start;
    s32 i;

    if (file_idx + 1 >= 9 || file_desc->x4C[file_idx + 1] == 0) {
        start = file_idx - 2;
    } else {
        start = file_idx - 1;
    }
    if (start < 0) {
        start = 0;
    }

    for (i = 0; i < 3 && i < 9; i++) {
        s32 idx = start + i;
        data[0] = idx;
        data[1] = ((((u32) file_desc->x4C[idx]) >> ((0, 0x10))) & 0x3F) |
                  ((file_desc->x28[idx] << 6) & 0xC0);
        data[2] = (u32) file_desc->x4C[idx] >> 8;
        data[3] = file_desc->x4C[idx];
        data += 4;
    }

    for (; i < 3; i++) {
        data[0] = 0;
        data[1] = 0;
        data[2] = 0;
        data[3] = 0;
        data += 4;
    }
}

void hsd_803AC558(struct CardState* file_desc, u8* data)
{
    int i;

    for (i = 0; i < 3; i++) {
        u8 file_idx = *data++;
        u8 byte1 = data[0];
        u32 size = (u32) data[1] | ((byte1 << 8) & 0x3F00);
        u32 final_size;
        u8 flags = (byte1 >> 6) & 3;
        size <<= 8;
        final_size = size | data[2];
        data += 3;

        if (final_size != 0 && file_desc->x4C[file_idx] == 0) {
            file_desc->x28[file_idx] = flags;
            file_desc->x4C[file_idx] = final_size;
        }
    }
}

u32 fn_803AC634(struct CardState* file_desc, s32 file_idx)
{
    if (file_desc->x4C[file_idx] <= 0) {
        return 0;
    }

    if (file_idx == 0) {
        u32 usable;
        u32 sector_size = file_desc->x8;
        s32 remaining;

        remaining = file_desc->x4C[0];
        remaining = remaining - (s32) ((usable = sector_size - 0x20) -
                                       (file_desc->x24 + 48) % sector_size);
        if (remaining <= 0) {
            return 1;
        }
        return (u32) (remaining + sector_size - 0x21) / usable + 1;
    }

    {
        u32 sector_size = file_desc->x8;
        return (u32) (file_desc->x4C[file_idx] + sector_size - 0x21) /
               (sector_size - 0x20);
    }
}

inline u32 fn_803AC6B8_first_block_count(struct CardState* file_desc)
{
    if (file_desc->x4C[0] <= 0) {
        return 0;
    }

    {
        u32 usable;
        u32 sector_size = file_desc->x8;
        s32 remaining;

        remaining = file_desc->x4C[0];
        remaining = remaining - (s32) ((usable = sector_size - 0x20) -
                                       (file_desc->x24 + 48) % sector_size);
        if (remaining <= 0) {
            return 1;
        }
        return (u32) (remaining + sector_size - 0x21) / usable + 1;
    }
}

s32 fn_803AC6B8(struct CardState* file_desc, s32 file_count)
{
    u32 sector_size;
    s32 remaining;
    u32 usable;
    s32 blocks;
    s32 file_size;
    s32 total;
    s32 i;

    if (file_count >= 9) {
        return 0;
    }
    if (file_count == 0) {
        return 0;
    }

    total = 1;
    if (file_desc->x4C[0] > 0) {
        total += fn_803AC6B8_first_block_count(file_desc) - 1;
    }

    for (i = 1; i < file_count; i++) {
        file_size = file_desc->x4C[i];
        if (file_size <= 0) {
            blocks = 0;
        } else if (i == 0) {
            sector_size = file_desc->x8;
            remaining = file_desc->x4C[0] -
                        (s32) ((usable = sector_size - 0x20) -
                               (file_desc->x24 + 48) % sector_size);
            if (remaining <= 0) {
                blocks = 1;
            } else {
                blocks = (u32) (remaining + sector_size - 0x21) / usable + 1;
            }
        } else {
            sector_size = file_desc->x8;
            blocks =
                (u32) (file_size + sector_size - 0x21) / (sector_size - 0x20);
        }
        total += blocks;
    }

    return total;
}

static inline s32 calculateBlocksBefore(struct CardState* file_desc,
                                        s32 file_count)
{
    s32 total;
    s32 i;

    if (file_count >= 9) {
        return 0;
    }
    if (file_count == 0) {
        return 0;
    }

    total = 1;
    if (file_desc->x4C[0] > 0) {
        total = fn_803AC634(file_desc, 0);
    }

    for (i = 1; i < file_count; i++) {
        total += fn_803AC634(file_desc, i);
    }

    return total;
}

inline u32 fn_803AC7DC_block_count(struct CardState* file_desc, s32 file_idx)
{
    if (file_desc->x4C[file_idx] <= 0) {
        return 0;
    }

    if (file_idx == 0) {
        u32 usable;
        u32 sector_size = file_desc->x8;
        s32 remaining;

        remaining = file_desc->x4C[0];
        remaining = remaining - (s32) ((usable = sector_size - 0x20) -
                                       (file_desc->x24 + 48) % sector_size);
        if (remaining <= 0) {
            return 1;
        }
        return (u32) (remaining + sector_size - 0x21) / usable + 1;
    }

    {
        u32 sector_size = file_desc->x8;
        return (u32) (file_desc->x4C[file_idx] + sector_size - 0x21) /
               (sector_size - 0x20);
    }
}

s32 fn_803AC7DC(CardState* state)
{
    s32 total = 0;
    s32 extra = 0;
    s32 i;

    if (state->x4C[0] > 0) {
        total = (s32) fn_803AC7DC_block_count(state, 0) - 1;
    }

    for (i = 1; i < 9; i++) {
        s32 size = state->x4C[i];

        if (size <= 0) {
            continue;
        }

        switch (state->x28[i]) {
        case 0:
            total += fn_803AC7DC_block_count(state, i) * 2;
            break;
        case 1:
            total += fn_803AC7DC_block_count(state, i);
            if (extra < (s32) fn_803AC7DC_block_count(state, i)) {
                extra = (s32) fn_803AC7DC_block_count(state, i);
            }
            break;
        case 2:
            total += fn_803AC7DC_block_count(state, i);
            if (extra < 1) {
                extra = 1;
            }
            break;
        case 3:
            total += fn_803AC7DC_block_count(state, i);
            break;
        }
    }

    return total + extra;
}

inline int fn_803ACB74(s32 seq_a, s32 seq_b)
{
    if (seq_a < 0) {
        return -1;
    }
    if (seq_b < 0) {
        return 1;
    }
    if (seq_a == 0 && seq_b == 0xFF) {
        return 1;
    }
    if (seq_a == 0xFF && seq_b == 0) {
        return -1;
    }
    if (seq_a - seq_b > 0x80) {
        return -1;
    }
    if (seq_a - seq_b < -0x80) {
        return 1;
    }
    return seq_a - seq_b;
}

s32 fn_803ACBE8(CardState* state, s32 block_idx)
{
    u32 size = state->x8;
    u32 temp = state->x24 + size;
    u32 num = temp + 0x2F;

    temp = num / size;
    {
        u32 idx = temp - 1;
        idx = block_idx + idx;
        return size * idx;
    }
}
s32 fn_803ACC0C(CardState* state, s32 block_idx, s32 file_id, s32 seq_num,
                void* expected_data, s32 data_size)
{
    s32 result;
    s32 retries;
    u8* buf;
    u32 sector_size;
    u8* data;
    u32 read_offset;
    s32 offset;
    PAD_STACK(4);

    if (data_size == 0) {
        return 0;
    }

    sector_size = state->x8;
    retries = 0;
    buf = state->x0;
    {
        u32 temp = state->x24 + sector_size;
        u32 num = temp + 0x2F;

        temp = num / sector_size;
        {
            u32 idx = temp - 1;
            idx = block_idx + idx;
            read_offset = sector_size * idx;
        }
    }

    for (; retries < 10; retries++) {
        {
            s32 tmp =
                CARDRead(&state->file_info, buf, sector_size, read_offset);
            result = tmp;
        }
        if (result != -1) {
            break;
        }
    }

    if (result < 0) {
        return result;
    }

    if (block_idx == 0) {
        offset = (state->x24 + 48) % state->x8;
    } else {
        offset = 0;
    }

    if (hsd_803B31CC(state->x0 + offset, state->x8 - offset) < 0) {
        return 1;
    }

    data = state->x0 + offset;
    if (file_id != ((data[0x10] << 8) | data[0x11])) {
        return 1;
    }

    if (seq_num != data[0x12]) {
        return 1;
    }

    if (memcmp(expected_data, data + 0x20, data_size) != 0) {
        return 1;
    }

    return 0;
}

s32 fn_803ACD58(CardState* state, void* icon_data, void* file_data)
{
    s32 icon_size;
    s32 i;
    s32 hdr_plus_icon;

    switch (state->x3B0) {
    case 2:
        icon_size = 0x1800;
        break;
    case 1:
        icon_size = 0xE00;
        break;
    default:
        icon_size = 0;
        break;
    }

    hdr_plus_icon = icon_size + 0x40;

    {
        s32 retries;
        u8* buf;
        s32 offset;
        s32 size;
        for (i = 0; size = state->x8, i < (0x2F + state->x24 + size) / size;
             i++)
        {
            s32 result;

            offset = i * size;
            buf = state->x0;
            for (retries = 0; retries < 10; retries++) {
                result = CARDRead(&state->file_info, buf, size, offset);
                if (result != -1) {
                    break;
                }
            }

            if (result < 0) {
                return result;
            }

            if (i == 0) {
                if (memcmp(state->x0, state->x370, 0x40) != 0) {
                    return 1;
                }
                if (icon_size > 0) {
                    if (memcmp(state->x0 + 0x40, icon_data, icon_size) != 0) {
                        return 1;
                    }
                }
                {
                    u32 total_size = state->x24;
                    u32 ss = state->x8;
                    if (total_size > ss) {
                        if (memcmp(state->x0 + hdr_plus_icon, file_data,
                                   ss - hdr_plus_icon) != 0)
                        {
                            return 1;
                        }
                    } else {
                        if (memcmp(state->x0 + hdr_plus_icon, file_data,
                                   total_size - hdr_plus_icon) != 0)
                        {
                            return 1;
                        }
                    }
                }
            } else {
                u32 ss = state->x8;
                u32 remaining = state->x24 - i * ss;
                s32 data_offset = i * ss - 0x40 - icon_size;

                if (remaining > ss) {
                    if (memcmp(state->x0, (u8*) file_data + data_offset, ss) !=
                        0)
                    {
                        return 1;
                    }
                } else {
                    if (memcmp(state->x0, (u8*) file_data + data_offset,
                               remaining) != 0)
                    {
                        return 1;
                    }
                }
            }
        }
    }

    return 0;
}

s32 fn_803ACF30(CardState* state, s32 file_id, s32 seq_num, s32 version)
{
    s32 buf[9];
    s32 ret;
    u32 i;
    PAD_STACK(4);

    for (i = 0; i < (0x2F + state->x24 + state->x8) / state->x8; i++) {
        buf[0] = 11;
        buf[1] = (s32) state;
        buf[2] = i;
        buf[3] = file_id;
        buf[4] = seq_num;
        buf[5] = version;
        ret = fn_803AC168(buf);
        if (ret < 0) {
            return ret;
        }
    }
    return 0;
}

typedef u8* CardSectorPtr;

static inline u8* getBlockHeader(CardState* state, s32 hdr_offset)
{
    return (CardSectorPtr) (hdr_offset + (s32) state->x0);
}

static inline u8* getChecksumStart(s32 hdr_offset, CardState* state)
{
    return hdr_offset + state->x0;
}

s32 fn_803ACFC0(CardState* state, s32 block_idx, s32 file_id, s32 seq_num,
                void* payload, s32 payload_size, s32 version)
{
    u32 sector_size;
    s32 retries;
    u8* buf;
    s32 offset;
    s32 result;
    s32 hdr_offset;
    PAD_STACK(8);

    sector_size = state->x8;
    {
        u32 hdr_start = state->x24 + 48;
        offset = sector_size *
                 (block_idx + (hdr_start + sector_size - 1) / sector_size - 1);

        if (block_idx == 0) {
            if (file_id != 0) {
                return -257;
            }

            buf = state->x0;
            hdr_offset = hdr_start % sector_size;
            for (retries = 0; retries < 10; retries++) {
                result = CARDRead(&state->file_info, buf, sector_size, offset);
                if (result != -1) {
                    break;
                }
            }

            if (result < 0) {
                return result;
            }
        } else {
            hdr_offset = 0;
        }
    }

    if (payload_size > 0) {
        memcpy(&getBlockHeader(state, hdr_offset)[0x20], payload,
               payload_size);
    }

    {
        s32 remaining = (state->x8 - hdr_offset) - payload_size - 0x20;
        if (remaining != 0) {
            s32 off = payload_size + 0x20;
            s32 addr = (s32) state->x0;
            memset((CardSectorPtr) (hdr_offset + off + addr), 0, remaining);
        }
    }

    memset(state->x0 + hdr_offset, 0, 0x20);

    (state->x0 + hdr_offset)[0x10] = (u8) (file_id >> 8);
    (state->x0 + hdr_offset)[0x11] = (u8) file_id;
    (state->x0 + hdr_offset)[0x12] = (u8) seq_num;

    fn_803AC3F8(state, getBlockHeader(state, hdr_offset) + 0x13, version);
    hsd_803B2FA0(getChecksumStart(hdr_offset, state), state->x8 - hdr_offset);

    {
        s32 write_retries;
        u8* write_buf;
        u32 write_size;
        write_size = state->x8;
        write_retries = 0;
        write_buf = state->x0;
        for (; write_retries < 10; write_retries++) {
            result =
                CARDWrite(&state->file_info, write_buf, write_size, offset);
            if (result != -1) {
                break;
            }
        }
    }

    return result;
}

static inline s32 queueFinishCardCommands(CardState* state)
{
    s32 cmd[9];

    cmd[0] = 6;
    cmd[1] = (s32) state;
    return fn_803AC168(cmd);
}

static inline s32 queueCardCommand2(CardState* state, s32 block, void* data,
                                    s32 length, s32 offset)
{
    s32 cmd[9];

    cmd[0] = 2;
    cmd[1] = (s32) state;
    cmd[4] = block;
    cmd[5] = 0;
    cmd[6] = (s32) data;
    cmd[8] = length;
    cmd[7] = offset;
    return fn_803AC168(cmd);
}

static inline s32 queueCardCommand1(CardState* state, s32 logical, s32 phys,
                                    s32 file_id, void* data, s32 length,
                                    s32 offset)
{
    s32 cmd[9];

    cmd[0] = 1;
    cmd[1] = (s32) state;
    cmd[3] = logical;
    cmd[4] = phys;
    cmd[5] = 0;
    cmd[6] = (s32) data;
    cmd[8] = length;
    cmd[7] = offset;
    cmd[2] = file_id;
    return fn_803AC168(cmd);
}

static inline s32 queueDeleteBlockCommand(CardState* state, s32 block,
                                          s32 file_id)
{
    s32 ofs = fn_803ACBE8(state, block);

    if (block == 0) {
        return -0x101;
    }
    return queueCardCommand1(state, block, 0xFFFF, file_id, NULL, 0, ofs);
}

static inline s32 queueReadBlockCommand(CardState* state, s32 block)
{
    s32 cmd[9];
    s32 ofs = fn_803ACBE8(state, block);

    cmd[0] = 0xF;
    cmd[1] = (s32) state;
    cmd[3] = block;
    cmd[7] = ofs;
    return fn_803AC168(cmd);
}

static inline s32 queueWriteBlockCommand(CardState* state, s32 block,
                                         s32 logical, s32 seq)
{
    s32 cmd[9];
    s32 ofs = fn_803ACBE8(state, block);

    cmd[0] = 0x10;
    cmd[1] = (s32) state;
    cmd[3] = block;
    cmd[4] = logical;
    cmd[5] = seq;
    cmd[7] = ofs;
    return fn_803AC168(cmd);
}

static inline s32 queueCopyBlockCommands(CardState* state, s32 src, s32 dst,
                                         s32 logical, s32 seq)
{
    int result;

    if (src == 0 || dst == 0) {
        return -0x101;
    }
    result = queueReadBlockCommand(state, src);
    if (result < 0) {
        return result;
    }
    return queueWriteBlockCommand(state, dst, logical, seq);
}

static inline void markDuplicateBlocks(CardState* state, s32* newmap,
                                       s32 logical)
{
    s32 phys;

    if (newmap[logical] >= 0) {
        for (phys = 1; phys <= state->x460; phys++) {
            if (phys != newmap[logical] && state->x170[phys] == logical) {
                state->x170[phys] = -logical;
            }
        }
    }
}

s32 fn_803AD16C(CardState* state)
{
    s32 work[64];
    s32 seq[64];
    s32 newmap[64];
    int filemap[64];
    s32 chosen[64];
    s32 result;
    s32 file_id;
    s32 pass;
    s32 i;
    s32 phys;
    s32 blocks_before;
    s32 file_blocks;

    result = 0;
    for (i = 0; i <= state->x460; i++) {
        newmap[i] = -1;
        work[i] = state->x170[i];
        seq[i] = state->x270[i];
    }

    if (state->x460 != fn_803AC7DC(state)) {
        return -257;
    }

    for (file_id = 0; file_id < 9; file_id++) {
        file_blocks = fn_803AC634(state, file_id);
        if (file_blocks <= 0) {
            continue;
        }

        blocks_before = calculateBlocksBefore(state, file_id);

        for (i = 0; i < file_blocks; i++) {
            filemap[i] = -1;
            chosen[i] = -1;
        }

        for (pass = 0; pass <= state->x460; pass++) {
            s32 cur_seq = -1;

            for (phys = 0; phys <= state->x460; phys++) {
                if (work[phys] >= 0) {
                    s32 logical = work[phys] - blocks_before;
                    if (logical >= 0 && logical < file_blocks) {
                        if (cur_seq == -1 ||
                            fn_803ACB74(cur_seq, seq[phys]) > 0)
                        {
                            cur_seq = seq[phys];
                        }
                    }
                }
            }
            if (cur_seq < 0) {
                break;
            }

            for (phys = 0; phys <= state->x460; phys++) {
                s32 logical = work[phys] - blocks_before;
                if (0 <= logical && logical < file_blocks &&
                    cur_seq == seq[phys])
                {
                    filemap[logical] = phys;
                }
            }

            {
                s32 count = 0;
                for (i = 0; i < file_blocks; i++) {
                    if (filemap[i] >= 0 && cur_seq == seq[filemap[i]]) {
                        count++;
                    } else {
                        break;
                    }
                }
                if (count == file_blocks) {
                    for (i = 0; i < file_blocks; i++) {
                        chosen[i] = filemap[i];
                    }
                }
            }

            for (i = 0; i < file_blocks; i++) {
                if (filemap[i] >= 0) {
                    work[filemap[i]] = -0x7FFF;
                }
            }
        }

        if (chosen[0] >= 0) {
            for (i = 0; i < file_blocks; i++) {
                newmap[blocks_before + i] = chosen[i];
            }
        } else {
            for (i = 0; i < file_blocks; i++) {
                if (filemap[i] < 0) {
                    result = -0x103;
                } else {
                    newmap[blocks_before + i] = filemap[i];
                    if (result != -0x103) {
                        result = -0x104;
                    }
                }
            }
        }
    }

    for (file_id = 0; file_id < 9; file_id++) {
        if (state->x4C[file_id] <= 0 ||
            (state->x28[file_id] != 1 && state->x28[file_id] != 2))
        {
            continue;
        }

        file_blocks = fn_803AC634(state, file_id);
        blocks_before = calculateBlocksBefore(state, file_id);

        for (i = 0; i < file_blocks; i++) {
            markDuplicateBlocks(state, newmap, blocks_before + i);
        }
    }

    for (file_id = 0; file_id < 9; file_id++) {
        if (state->x4C[file_id] <= 0 || state->x28[file_id] != 0) {
            continue;
        }

        file_blocks = fn_803AC634(state, file_id);
        blocks_before = calculateBlocksBefore(state, file_id);

        for (i = 0; i < file_blocks; i++) {
            s32 logical = blocks_before + i;
            s32 src = newmap[logical];
            s32 dup;
            s32 target_seq;
            s32 ret;

            if (newmap[logical] < 0) {
                continue;
            }

            target_seq = state->x270[src];
            dup = -1;
            for (phys = 1; phys <= state->x460; phys++) {
                if (phys != newmap[logical] && state->x170[phys] == logical) {
                    if (dup < 0) {
                        dup = phys;
                    } else {
                        ret = queueDeleteBlockCommand(state, phys, file_id);
                        if (ret < 0 && result == 0) {
                            result = -0x10B;
                        }
                        state->x170[phys] = -0x7FFF;
                        state->x270[phys] = 0;
                    }
                }
            }

            if (dup == -1) {
                for (phys = state->x460; phys > 0; phys--) {
                    if (state->x170[phys] < 0) {
                        dup = phys;
                        break;
                    }
                }
                if (dup >= 0) {
                    ret = queueCopyBlockCommands(state, newmap[logical], dup,
                                                 logical, target_seq);
                    if (ret < 0 && (result == 0 || result == -0x10B)) {
                        result = -0x102;
                    }
                }
            } else if (state->x270[dup] != target_seq) {
                ret = queueCopyBlockCommands(state, newmap[logical], dup,
                                             logical, target_seq);
                if (ret < 0 && (result == 0 || result == -0x10B)) {
                    result = -0x102;
                }
            }
        }
    }

    return result;
}

s32 fn_803ADE4C(s32 card_state, s32 channel, s32 callback)
{
    s32 buf1[9];
    s32 buf2[9];
    s32 result;
    s32 saved;
    s32 snap;
    s32 saved1;
    s32 snap1;
{
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    PAD_STACK(16);

    hsd_804D7998 = hsd_804D7984;
    buf1[0] = 12;
    buf1[1] = card_state;
    buf1[2] = channel;
    result = fn_803AC168(buf1);
    if (result < 0) {
        snap1 = hsd_804D7998;
        if (snap1 >= 0) {
            saved1 = snap1;
            while (saved1 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved1].x10 = 0;
                saved1 = (saved1 + 1) % 128;
            }
            hsd_804D7984 = snap1;
        }
        return result;
    }

    buf2[0] = 17;
    buf2[1] = card_state;
    result = fn_803AC168(buf2);
    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            saved = snap;
            while (saved != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved].x10 = 0;
                saved = (saved + 1) % 128;
            }
            hsd_804D7984 = snap;
        }
        return result;
    }

    entries[0].x0 = 5;
    entries[0].x4 = card_state;
    entries[0].x8 = callback;
    entries[0].xC = 0;
    }
    hsd_804D7998 = -1;
    return 0;
}

static inline s32 calculateDataBlockSize(CardState* state, s32 file_idx,
                                         s32 block_idx)
{
    if (file_idx == 0 && block_idx == 0) {
        return (state->x8 - 0x20) - ((state->x24 + 0x30) % state->x8);
    }
    return state->x8 - 0x20;
}

static inline s32 calculateFileBlockCount(CardState* state, s32 file_idx)
{
    s32 file_size = state->x4C[file_idx];

    if (file_size <= 0) {
        return 0;
    } else if (file_idx == 0) {
        u32 sector_size = state->x8;
        u32 usable;
        s32 rem;

        rem = state->x4C[0];
        usable = sector_size - 0x20;
        rem = rem - (s32) (usable - (state->x24 + 0x30) % sector_size);
        if (rem <= 0) {
            return 1;
        } else {
            return (u32) (rem + sector_size - 0x21) / (sector_size - 0x20) + 1;
        }
    } else {
        u32 sector_size = state->x8;
        return (u32) (file_size + sector_size - 0x21) / (sector_size - 0x20);
    }
}

static inline s32 retryCardRead(CARDFileInfo* info, void* buffer, s32 length,
                                s32 offset)
{
    s32 retries;
    s32 result;

    for (retries = 0; retries < 10; retries++) {
        result = CARDRead(info, buffer, length, offset);
        if (result != -1) {
            break;
        }
    }
    return result;
}

static inline s32 retryCardWrite(CARDFileInfo* info, const void* buffer,
                                 s32 length, s32 offset)
{
    s32 retries;
    s32 result;

    for (retries = 0; retries < 10; retries++) {
        result = CARDWrite(info, buffer, length, offset);
        if (result != -1) {
            break;
        }
    }
    return result;
}

static inline void cancelQueuedCardCommands(CardBufEntry* entries)
{
    s32 saved = hsd_804D7998;

    if (saved >= 0) {
        s32 current = saved;

        while (current != hsd_804D7984) {
            entries[current].x10 = 0;
            current = (current + 1) % 128;
        }
        hsd_804D7984 = saved;
    }
}

static inline s32 queueClearDataBlock(CardState* state, u8* dst, s32 size)
{
    s32 cmd[9];

    cmd[0] = 4;
    cmd[1] = (s32) state;
    cmd[4] = 0;
    cmd[5] = 0;
    cmd[6] = (s32) dst;
    cmd[8] = size;
    cmd[7] = 0;
    cmd[2] = 0;
    return fn_803AC168(cmd);
}

static inline s32 queueDataBlock(CardState* state, s32 block_idx, u8* dst,
                                 s32 size)
{
    s32 offset = fn_803ACBE8(state, block_idx);
    return queueCardCommand2(state, block_idx, dst, size, offset);
}

static inline s32 readCardDataBlock(CardState* state, s32 data_block, u8* dst,
                                    s32 length)
{
    s32 offset;
    s32 read_ofs;
    int result;

    offset = fn_803ACBE8(state, data_block);
    result = retryCardRead(&state->file_info, state->x0, state->x8, offset);
    if (result < 0) {
        return result;
    }

    read_ofs = data_block == 0 ? (state->x24 + 0x30) % state->x8 : 0;
    if (hsd_803B31CC(state->x0 + read_ofs, state->x8 - read_ofs) < 0) {
        return -0x105;
    }
    if (length != 0 && dst != NULL) {
        memcpy(dst, state->x0 + (read_ofs + 0x20), length);
    }
    return 0;
}

s32 fn_803ADF90(struct CardState* arg0, s32 arg1, u8* arg2, s32 arg3,
                void (*arg4)(s32, s32))
{
    CardState* state = arg0;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    u8* dst;
    s32 remaining;
    s32 block_map[64];
    int blocks_before;
    int i;
    int logical;
    int scan_idx;
    int result;
    int file_blocks;
    s32 seq_num;
    s32 total_blocks;
    s32 file_size;
    s32 callback_seq;

    callback_seq = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = calculateBlocksBefore(arg0, arg1);

    file_size = arg0->x4C[arg1];
    file_blocks = calculateFileBlockCount(arg0, arg1);

    total_blocks = fn_803AC7DC(arg0);
    for (i = 0; i < file_blocks; i++) {
        block_map[i] = -1;
    }

    seq_num = -1;
    for (scan_idx = 1; scan_idx <= total_blocks; scan_idx++) {
        logical = arg0->x170[scan_idx] - blocks_before;
        if (logical >= 0 && logical < file_blocks) {
            if (seq_num < 0) {
                seq_num = arg0->x270[scan_idx];
            } else {
                if (seq_num != arg0->x270[scan_idx]) {
                    callback_seq = -260;
                }
                if (fn_803ACB74(seq_num, arg0->x270[scan_idx]) > 0) {
                    seq_num = arg0->x270[scan_idx];
                }
            }
            block_map[logical] = scan_idx;
        }
    }

    if (arg1 == 0 && arg0->x170[0] == 0) {
        block_map[0] = 0;
        if (seq_num >= 0 && seq_num != arg0->x270[0]) {
            callback_seq = -260;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        if (block_map[i] < 0) {
            callback_seq = -259;
            break;
        }
    }

    if (callback_seq != 0) {
        return callback_seq;
    }

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        result = retryCardFastOpen(arg0->x4, arg0->x20, &arg0->file_info);
        if (result < 0) {
            return result;
        }
    }

    dst = arg2;
    remaining = file_size;

    for (i = 0; i < file_blocks && remaining > 0; i++) {
        s32 chunk;

        chunk = calculateDataBlockSize(arg0, arg1, i);

        if (remaining > chunk) {
            s32 data_block = block_map[i];
            if (data_block >= 0) {
                if (arg3 != 0) {
                    result = queueDataBlock(arg0, data_block, dst, chunk);
                    if (result < 0) {
                        cancelQueuedCardCommands(entries);
                        return result;
                    }
                } else {
                    result = readCardDataBlock(arg0, data_block, dst, chunk);
                    if (result < 0) {
                        if (result == -0x105) {
                            callback_seq = -259;
                        } else {
                            callback_seq = -259;
                        }
                    }
                }
            } else if (arg3 != 0) {
                if (chunk == 0) {
                    result = 0;
                } else {
                    result = queueClearDataBlock(arg0, dst, chunk);
                }
                if (result < 0) {
                    cancelQueuedCardCommands(entries);
                    return result;
                }
            } else {
                memset(dst, 0, chunk);
            }

            remaining -= chunk;
            dst += chunk;
        } else {
            s32 data_block = block_map[i];
            if (data_block >= 0) {
                if (arg3 != 0) {
                    result = queueDataBlock(arg0, data_block, dst, remaining);
                    if (result < 0) {
                        cancelQueuedCardCommands(entries);
                        return result;
                    }
                } else {
                    result =
                        readCardDataBlock(arg0, data_block, dst, remaining);
                    if (result < 0) {
                        if (result == -0x105) {
                            callback_seq = -259;
                        } else {
                            callback_seq = -259;
                        }
                    }
                }
            }
            remaining = 0;
        }
    }

    if (arg3 == 0) {
        result = retryCardClose(&arg0->file_info);
        if (result < 0) {
            return -267;
        }
    } else {
        entries[0].x0 = 1;
        entries[0].x4 = (s32) arg0;
        entries[0].x8 = (s32) arg4;
        entries[0].xC = arg1;
        hsd_804D7998 = -1;
    }

    return callback_seq;
}

static inline void rollbackCardCommands(CardContext* context, s32 snap)
{
    s32 saved = snap;

    while (saved != hsd_804D7984) {
        ((CardBufEntry*) context)[saved].x10 = 0;
        saved = (saved + 1) % 128;
    }
    hsd_804D7984 = snap;
}

static inline s32 queueVerifyBlockCommand(CardState* state, s32 block,
                                          s32 logical, s32 seq, void* data,
                                          s32 length)
{
    s32 cmd[9];
    s32 offset = fn_803ACBE8(state, block);

    cmd[0] = 5;
    cmd[1] = (s32) state;
    cmd[4] = logical;
    cmd[5] = seq;
    cmd[6] = (s32) data;
    cmd[8] = length;
    cmd[7] = offset;
    return fn_803AC168(cmd);
}

static inline s32 queueWriteFileBlockCommand(CardState* state, s32 block,
                                             s32 logical, s32 seq, void* data,
                                             s32 length, s32 file_idx,
                                             s32 offset)
{
    s32 cmd[9];

    cmd[0] = 1;
    cmd[1] = (s32) state;
    cmd[3] = block;
    cmd[4] = logical;
    cmd[5] = seq;
    cmd[6] = (s32) data;
    cmd[7] = offset;
    cmd[8] = length;
    cmd[2] = file_idx;
    return fn_803AC168(cmd);
}

s32 fn_803AE7F8(struct CardState* arg0, s32 arg1, u8* arg2, s32 arg3,
                void (*arg4)(s32, s32))
{
    CardContext* context = (CardContext*) hsd_804D1138;
    s32 snap;
    s32 block_map[3][64];
    s32* block_map_ptr;
    s32 blocks_before;
    s32 current_seq;
    s32 chunk;
    s32 file_blocks;
    s32 file_size;
    s32 free_count;
    s32 i;
    int logical;
    s32 repair_result;
    s32 result;
    s32 total_blocks;
    s32 verify_failed;
    s32 retries;
    u8* data;

    repair_result = 0;
    verify_failed = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = calculateBlocksBefore(arg0, arg1);

    file_size = arg0->x4C[arg1];
    file_blocks = calculateFileBlockCount(arg0, arg1);
    total_blocks = fn_803AC7DC(arg0);

    for (i = 0; i < file_blocks; i++) {
        block_map[0][i] = -1;
        block_map[1][i] = -1;
    }

    free_count = 0;
    current_seq = -1;

    {
        s32 block_idx;

        for (block_idx = 1; block_idx <= total_blocks; block_idx++) {
            s32 file_idx = arg0->x170[block_idx];

            if (file_idx < 0) {
                block_map[2][free_count++] = block_idx;
                continue;
            }

            logical = file_idx - blocks_before;
            if (logical >= 0 && logical < file_blocks) {
                if (current_seq == -1 ||
                    fn_803ACB74(current_seq, arg0->x270[block_idx]) < 0)
                {
                    current_seq = arg0->x270[block_idx];
                }
                if (block_map[0][logical] < 0) {
                    block_map[0][logical] = block_idx;
                } else {
                    block_map[1][logical] = block_idx;
                }
            }
        }
    }

    block_map_ptr = block_map[0];
    for (i = 0; i < file_blocks; i++) {
        if (block_map_ptr[i] < 0 ||
            arg0->x270[block_map_ptr[i]] != current_seq)
        {
            verify_failed = 1;
            break;
        }
        if (block_map_ptr[64 + i] < 0 ||
            arg0->x270[block_map_ptr[64 + i]] != current_seq)
        {
            verify_failed = 1;
            break;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        if (block_map_ptr[i] < 0) {
            if (free_count > 0) {
                block_map_ptr[i] = block_map[2][--free_count];
            } else {
                return -257;
            }
        }
        if (block_map_ptr[64 + i] < 0) {
            if (free_count > 0) {
                block_map_ptr[64 + i] = block_map[2][--free_count];
            } else {
                repair_result = -258;
            }
        }
    }

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        s32 fd;
        s32 ofs;
        s32 open_result;

        ofs = arg0->x20;
        retries = 0;
        fd = arg0->x4;
        do {
            open_result = CARDFastOpen(fd, ofs, &arg0->file_info);
            if (open_result != -1) {
                break;
            }
            retries++;
        } while (retries < 10);

        if (open_result < 0) {
            return open_result;
        }
    }

    if (verify_failed == 0) {
        s32* map;
        s32 pass;

        pass = 0;
        map = block_map_ptr;
        for (; pass < 2; pass++, map += 64) {
            s32 remaining = file_size;

            data = arg2;
            for (i = 0; i < file_blocks && remaining > 0; i++) {
                s32 phys;
                chunk = arg0->x8 - 0x20;

                if ((u32) remaining > (u32) chunk) {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 cmd_result;
                            if (chunk == 0) {
                                cmd_result = 0;
                            } else {
                                cmd_result = queueVerifyBlockCommand(
                                    arg0, phys, blocks_before + i, current_seq,
                                    data, chunk);
                            }
                            if (cmd_result < 0) {
                                snap = hsd_804D7998;
                                if (snap >= 0) {
                                    rollbackCardCommands(context, snap);
                                }
                                return cmd_result;
                            }
                        } else {
                            result = fn_803ACC0C(arg0, phys, blocks_before + i,
                                                 current_seq, data, chunk);
                            if (result < 0) {
                                retryCardClose(&arg0->file_info);
                                return result;
                            }
                            if (result > 0) {
                                goto write;
                            }
                        }
                    }

                    remaining -= arg0->x8 - 0x20;
                    data += arg0->x8 - 0x20;
                } else {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 cmd_result;
                            if (remaining == 0) {
                                cmd_result = 0;
                            } else {
                                cmd_result = queueVerifyBlockCommand(
                                    arg0, phys, blocks_before + i, current_seq,
                                    data, remaining);
                            }
                            if (cmd_result < 0) {
                                snap = hsd_804D7998;
                                if (snap >= 0) {
                                    rollbackCardCommands(context, snap);
                                }
                                return cmd_result;
                            }
                        } else {
                            result = fn_803ACC0C(arg0, phys, blocks_before + i,
                                                 current_seq, data, remaining);
                            if (result < 0) {
                                retryCardClose(&arg0->file_info);
                                return result;
                            }
                            if (result > 0) {
                                goto write;
                            }
                        }
                    }

                    remaining = 0;
                }
            }
        }

        if (arg3 != 0) {
            s32 cmd_result;

            cmd_result = queueFinishCardCommands(arg0);
            if (cmd_result < 0) {
                snap = hsd_804D7998;
                if (snap >= 0) {
                    rollbackCardCommands(context, snap);
                }
                return cmd_result;
            }
        } else {
            result = retryCardClose(&arg0->file_info);

            if (result < 0) {
                return -267;
            }
            return 1;
        }
    }

write:
    current_seq = (u8) (current_seq + 1);
    {
        s32 pass;
        s32* map = block_map_ptr;

        for (pass = 0; pass < 2; pass++, map += 64) {
            s32 remaining = file_size;

            data = arg2;
            for (i = 0; i < file_blocks && remaining > 0; i++) {
                s32 phys;
                chunk = arg0->x8 - 0x20;

                if ((u32) remaining > (u32) chunk) {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 ofs = fn_803ACBE8(arg0, phys);
                            s32 cmd_result;
                            if (phys == 0) {
                                if (blocks_before + i != 0) {
                                    cmd_result = -0x101;
                                    goto write_chunk_done;
                                }
                                queueCardCommand2(arg0, 0, NULL, 0, ofs);
                            }
                            cmd_result = queueWriteFileBlockCommand(
                                arg0, phys, blocks_before + i, current_seq,
                                data, chunk, arg1, ofs);
                        write_chunk_done:
                            if (cmd_result < 0) {
                                snap = hsd_804D7998;
                                if (snap >= 0) {
                                    rollbackCardCommands(context, snap);
                                }
                                return cmd_result;
                            }
                        } else {
                            result =
                                fn_803ACFC0(arg0, phys, blocks_before + i,
                                            current_seq, data, chunk, arg1);
                            if (result < 0) {
                                arg0->x170[block_map[pass][i]] = -0x7FFF;
                                arg0->x270[block_map[pass][i]] = 0;
                                retryCardClose(&arg0->file_info);
                                return pass == 0 ? -260 : -258;
                            }
                        }

                        arg0->x170[map[i]] = blocks_before + i;
                        arg0->x270[map[i]] = current_seq;
                    }

                    remaining -= arg0->x8 - 0x20;
                    data += arg0->x8 - 0x20;
                } else {
                    phys = map[i];
                    if (phys >= 0) {
                        if (arg3 != 0) {
                            s32 ofs = fn_803ACBE8(arg0, phys);
                            s32 cmd_result;
                            if (phys == 0) {
                                if (blocks_before + i != 0) {
                                    cmd_result = -0x101;
                                    goto write_remaining_done;
                                }
                                queueCardCommand2(arg0, 0, NULL, 0, ofs);
                            }
                            cmd_result = queueWriteFileBlockCommand(
                                arg0, phys, blocks_before + i, current_seq,
                                data, remaining, arg1, ofs);
                        write_remaining_done:
                            if (cmd_result < 0) {
                                snap = hsd_804D7998;
                                if (snap >= 0) {
                                    rollbackCardCommands(context, snap);
                                }
                                return cmd_result;
                            }
                        } else {
                            result = fn_803ACFC0(arg0, phys, blocks_before + i,
                                                 current_seq, data, remaining,
                                                 arg1);
                            if (result < 0) {
                                arg0->x170[block_map[pass][i]] = -0x7FFF;
                                arg0->x270[block_map[pass][i]] = 0;
                                retryCardClose(&arg0->file_info);
                                return pass == 0 ? -260 : -258;
                            }
                        }

                        arg0->x170[map[i]] = blocks_before + i;
                        arg0->x270[map[i]] = current_seq;
                    }

                    remaining = 0;
                }
            }
        }
    }

    if (arg3 == 0) {
        result = retryCardClose(&arg0->file_info);

        if (result < 0) {
            return repair_result == 0 ? -267 : repair_result;
        }
    } else {
        context->x0 = 2;
        context->x4 = arg0;
        context->x8 = arg4;
        context->xC = arg1;
        hsd_804D7998 = -1;
    }

    return repair_result;
}

static inline s32 queueWriteDataBlock(CardState* state, s32 block, s32 logical,
                                      s32 file_id, s32 seq, u8* data,
                                      s32 length)
{
    s32 seek_cmd[9];
    s32 cmd[9];
    s32 ofs = fn_803ACBE8(state, block);

    if (block == 0) {
        if (logical != 0) {
            return -0x101;
        }
        seek_cmd[0] = 2;
        seek_cmd[1] = (s32) state;
        seek_cmd[4] = 0;
        seek_cmd[5] = 0;
        seek_cmd[6] = 0;
        seek_cmd[8] = 0;
        seek_cmd[7] = ofs;
        fn_803AC168(seek_cmd);
    }

    cmd[0] = 1;
    cmd[1] = (s32) state;
    cmd[2] = file_id;
    cmd[3] = block;
    cmd[4] = logical;
    cmd[5] = seq;
    cmd[6] = (s32) data;
    cmd[7] = ofs;
    cmd[8] = length;
    return fn_803AC168(cmd);
}
static inline s32 queueWriteRemainingDataBlock(CardState* state, s32 block,
                                               s32 logical, s32 file_id,
                                               s32 seq, u8* data, s32 length)
{
    s32 ofs = fn_803ACBE8(state, block);

    if (block == 0) {
        if (logical != 0) {
            return -0x101;
        }
        queueCardCommand2(state, 0, NULL, 0, ofs);
    }
    return queueWriteFileBlockCommand(state, block, logical, seq, data, length,
                                      file_id, ofs);
}

s32 fn_803AF3F0(struct CardState* arg0, s32 arg1, u8* arg2, s32 arg3,
                void (*arg4)(s32, s32))
{
    s32 snap;
    CardContext* context = (CardContext*) hsd_804D1138;
    int block_map[3][64];
    s32 blocks_before;
    s32 file_blocks;
    s32 file_size;
    s32 total_blocks;
    s32 current_seq;
    s32 chunk;
    s32 seq_match;
    s32 secondary_count;
    s32 free_count;
    s32 i;
    s32 remaining;
    s32 result;
    u8* data;

    seq_match = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = calculateBlocksBefore(arg0, arg1);

    file_size = arg0->x4C[arg1];
    file_blocks = calculateFileBlockCount(arg0, arg1);
    total_blocks = fn_803AC7DC(arg0);

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        result = retryCardFastOpen(arg0->x4, arg0->x20, &arg0->file_info);
        if (result < 0) {
            return result;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        block_map[0][i] = -1;
        block_map[1][i] = -1;
    }

    current_seq = -1;
    secondary_count = 0;
    free_count = 0;
    for (i = 1; i <= total_blocks; i++) {
        s32 arg1x = arg0->x170[i];

        if (arg1x >= 0) {
            int logical = arg1x - blocks_before;
            if (logical >= 0 && logical < file_blocks) {
                if (fn_803ACB74(current_seq, arg0->x270[i]) < 0) {
                    current_seq = arg0->x270[i];
                }
                block_map[0][logical] = i;
            }
        } else {
            s32 logical = -(arg1x + blocks_before);
            if (logical >= 0 && logical < file_blocks) {
                block_map[1][secondary_count] = i;
                secondary_count++;
            } else {
                block_map[2][free_count] = i;
                free_count++;
            }
        }
    }

    for (i = 0; i < file_blocks; i++) {
        if (block_map[0][i] < 0 || arg0->x270[block_map[0][i]] != current_seq)
        {
            seq_match = 1;
            break;
        }
    }

    if (seq_match == 0) {
        remaining = file_size;
        data = arg2;
        for (i = 0; i < file_blocks && remaining > 0; i++) {
            if ((u32) remaining > (u32) (chunk = arg0->x8 - 0x20)) {
                if (arg3 != 0) {
                    s32 result;
                    s32 block = block_map[0][i];
                    if (chunk == 0) {
                        result = 0;
                    } else {
                        result = queueVerifyBlockCommand(
                            arg0, block, blocks_before + i, current_seq, data,
                            chunk);
                    }
                    if (result < 0) {
                        snap = hsd_804D7998;
                        if (snap >= 0) {
                            rollbackCardCommands(context, snap);
                        }
                        return result;
                    }
                } else {
                    result =
                        fn_803ACC0C(arg0, block_map[0][i], blocks_before + i,
                                    current_seq, data, chunk);
                    if (result < 0) {
                        retryCardClose(&arg0->file_info);
                        return result;
                    }
                    if (result > 0) {
                        goto repair;
                    }
                }
                remaining -= arg0->x8 - 0x20;
                data += arg0->x8 - 0x20;
            } else {
                if (arg3 != 0) {
                    s32 result;
                    s32 block = block_map[0][i];
                    if (remaining == 0) {
                        result = 0;
                    } else {
                        result = queueVerifyBlockCommand(
                            arg0, block, blocks_before + i, current_seq, data,
                            remaining);
                    }
                    if (result < 0) {
                        snap = hsd_804D7998;
                        if (snap >= 0) {
                            rollbackCardCommands(context, snap);
                        }
                        return result;
                    }
                } else {
                    result =
                        fn_803ACC0C(arg0, block_map[0][i], blocks_before + i,
                                    current_seq, data, remaining);
                    if (result < 0) {
                        retryCardClose(&arg0->file_info);
                        return result;
                    }
                    if (result > 0) {
                        goto repair;
                    }
                }
                remaining = 0;
            }
        }

        if (arg3 != 0) {
            s32 result;
            result = queueFinishCardCommands(arg0);
            if (result < 0) {
                snap = hsd_804D7998;
                if (snap >= 0) {
                    rollbackCardCommands(context, snap);
                }
                return result;
            }
        } else {
            result = retryCardClose(&arg0->file_info);
            if (result < 0) {
                return -267;
            }
            return 1;
        }
    }

repair:
    if (secondary_count > file_blocks) {
        for (i = file_blocks; i < secondary_count; i++) {
            if (arg3 != 0) {
                s32 result;
                s32 block = block_map[1][i];
                result = queueDeleteBlockCommand(arg0, block, arg1);
                if (result < 0) {
                    snap = hsd_804D7998;
                    if (snap >= 0) {
                        rollbackCardCommands(context, snap);
                    }
                    return result;
                }
            } else {
                result = fn_803ACFC0(arg0, block_map[1][i], 0xFFFF, 0, NULL, 0,
                                     arg1);
                arg0->x170[block_map[1][i]] = -0x7FFF;
                arg0->x270[block_map[1][i]] = 0;
                if (result < 0) {
                    retryCardClose(&arg0->file_info);
                    return result;
                }
            }
        }
    } else if (secondary_count < file_blocks) {
        while (secondary_count < file_blocks && free_count > 0) {
            block_map[1][secondary_count++] = block_map[2][--free_count];
        }
        if (secondary_count < file_blocks) {
            for (i = 0; secondary_count < file_blocks && i < file_blocks; i++)
            {
                if (block_map[0][i] >= 0) {
                    block_map[1][secondary_count++] = block_map[0][i];
                    block_map[0][i] = -1;
                }
            }
        }
        if (secondary_count < file_blocks) {
            if (arg3 == 0) {
                retryCardClose(&arg0->file_info);
            }
            return -257;
        }
    }

    current_seq = (u8) (current_seq + 1);
    remaining = file_size;
    data = arg2;
    for (i = 0; i < file_blocks && remaining > 0; i++) {
        chunk = arg0->x8 - 0x20;

        if ((u32) remaining > (u32) chunk) {
            if (arg3 != 0) {
                s32 result;
                s32 block = block_map[1][i];
                result = queueWriteDataBlock(arg0, block, blocks_before + i,
                                             arg1, current_seq, data, chunk);
                if (result < 0) {
                    snap = hsd_804D7998;
                    if (snap >= 0) {
                        rollbackCardCommands(context, snap);
                    }
                    return result;
                }
            } else {
                result = fn_803ACFC0(arg0, block_map[1][i], blocks_before + i,
                                     current_seq, data, chunk, arg1);
                if (result < 0) {
                    arg0->x170[block_map[1][i]] = -0x7FFF;
                    arg0->x270[block_map[1][i]] = 0;
                    {
                        retryCardClose(&arg0->file_info);
                    }
                    return result;
                }
                arg0->x170[block_map[1][i]] = blocks_before + i;
                arg0->x270[block_map[1][i]] = current_seq;
            }
            remaining -= arg0->x8 - 0x20;
            data += arg0->x8 - 0x20;
        } else {
            if (arg3 != 0) {
                s32 result;
                s32 block = block_map[1][i];
                s32 ofs = fn_803ACBE8(arg0, block);
                if (block == 0) {
                    if (blocks_before + i != 0) {
                        result = -0x101;
                        goto write_remaining_done;
                    }
                    queueCardCommand2(arg0, 0, NULL, 0, ofs);
                }
                result = queueWriteFileBlockCommand(
                    arg0, block, blocks_before + i, current_seq, data,
                    remaining, arg1, ofs);
            write_remaining_done:
                if (result < 0) {
                    snap = hsd_804D7998;
                    if (snap >= 0) {
                        rollbackCardCommands(context, snap);
                    }
                    return result;
                }
            } else {
                s32 write_result =
                    fn_803ACFC0(arg0, block_map[1][i], blocks_before + i,
                                current_seq, data, remaining, arg1);
                if (write_result < 0) {
                    arg0->x170[block_map[1][i]] = -0x7FFF;
                    arg0->x270[block_map[1][i]] = 0;
                    {
                        retryCardClose(&arg0->file_info);
                    }
                    return write_result;
                }
                arg0->x170[block_map[1][i]] = blocks_before + i;
                arg0->x270[block_map[1][i]] = current_seq;
            }
            remaining = 0;
        }
    }

    if (arg3 == 0) {
        for (i = 0; i < file_blocks; i++) {
            if (block_map[0][i] >= 0) {
                arg0->x170[block_map[0][i]] = -arg0->x170[block_map[0][i]];
            }
        }
        result = retryCardClose(&arg0->file_info);
        if (result < 0) {
            return -267;
        }
    } else {
        context->x0 = 3;
        context->x4 = arg0;
        context->x8 = arg4;
        context->xC = arg1;
        hsd_804D7998 = -1;
    }

    return 0;
}

s32 fn_803B0120(CardState* arg0, s32 arg1, u8* arg2, s32 arg3,
                void (*arg4)(s32, s32))
{
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    s32 block_map[3][64];
    s32 blocks_before;
    s32 secondary_count;
    s32 file_size;
    s32 total_blocks;
    s32 current_seq;
    s32 chunk;
    s32 needs_rewrite;
    s32 file_blocks;
    s32 free_count;
    s32 i;
    s32 remaining;
    s32 result;
    u8* data;
    needs_rewrite = 0;
    if (arg3 == 0) {
        BOOL intr = OSDisableInterrupts();
        s32 busy;
        if (hsd_804D799C == 1) {
            busy = 1;
        } else {
            busy = 0;
        }
        OSRestoreInterrupts(intr);
        if (busy) {
            return -264;
        }
    }

    blocks_before = calculateBlocksBefore(arg0, arg1);

    file_size = arg0->x4C[arg1];
    file_blocks = calculateFileBlockCount(arg0, arg1);
    total_blocks = fn_803AC7DC(arg0);

    if (arg3 != 0) {
        hsd_804D7998 = hsd_804D7984;
    } else {
        result = retryCardFastOpen(arg0->x4, arg0->x20, &arg0->file_info);
        if (result < 0) {
            return result;
        }
    }

    for (i = 0; i < file_blocks; i++) {
        block_map[0][i] = -1;
        block_map[1][i] = -1;
    }

    current_seq = -1;
    if (arg1 == 0) {
        if (arg0->x170[0] == 0) {
            current_seq = arg0->x270[0];
        } else {
            needs_rewrite = 1;
        }
        block_map[0][0] = 0;
    }

    secondary_count = 0;
    free_count = 0;
    for (i = 1; i <= total_blocks; i++) {
        s32 file_idx = arg0->x170[i];

        if (file_idx >= 0) {
            s32 logical = file_idx - blocks_before;
            if (arg1 == 0 && logical == 0) {
                block_map[1][secondary_count] = i;
                secondary_count++;
            } else if (logical >= 0 && logical < file_blocks) {
                s32 seq = arg0->x270[i];
                if (fn_803ACB74(current_seq, seq) < 0) {
                    current_seq = seq;
                }
                block_map[0][logical] = i;
            }
        } else {
            s32 logical = -(file_idx + blocks_before);
            if (logical >= 0 && logical < file_blocks) {
                block_map[1][secondary_count] = i;
                secondary_count++;
            } else {
                block_map[2][free_count] = i;
                free_count++;
            }
        }
    }

    for (i = 0; i < file_blocks; i++) {
        if (block_map[0][i] < 0) {
            needs_rewrite = 1;
            if (secondary_count > 0) {
                secondary_count--;
                block_map[0][i] = block_map[1][secondary_count];
                block_map[1][secondary_count] = -1;
            } else if (free_count > 0) {
                free_count--;
                block_map[0][i] = block_map[2][free_count];
                block_map[2][free_count] = -1;
            } else {
                if (arg3 == 0) {
                    retryCardClose(&arg0->file_info);
                }
                return -257;
            }
        }
    }

    if (needs_rewrite == 0) {
        for (i = 0; i < file_blocks; i++) {
            if (block_map[0][i] < 0 ||
                arg0->x270[block_map[0][i]] != current_seq)
            {
                needs_rewrite = 1;
                break;
            }
        }
    }

    if (needs_rewrite != 0) {
        goto writes_done;
    }
    remaining = file_size;
    data = arg2;
    for (i = 0; i < file_blocks && remaining > 0; i++) {
        s32 logical = blocks_before + i;
        if (logical == 0) {
            chunk = (arg0->x8 - 0x20) - ((arg0->x24 + 0x30) % arg0->x8);
        } else {
            chunk = arg0->x8 - 0x20;
        }

        if (remaining > chunk) {
            if (arg3 != 0) {
                s32 block = block_map[0][i];
                s32 cmd_result;
                if (chunk == 0) {
                    cmd_result = 0;
                } else {
                    cmd_result =
                        queueVerifyBlockCommand(arg0, block, blocks_before + i,
                                                current_seq, data, chunk);
                }
                if (cmd_result < 0) {
                    cancelQueuedCardCommands(entries);
                    return cmd_result;
                }
            } else {
                result = fn_803ACC0C(arg0, block_map[0][i], blocks_before + i,
                                     current_seq, data, chunk);
                if (result < 0) {
                    retryCardClose(&arg0->file_info);
                    return result;
                }
                if (result > 0) {
                    goto writes_done;
                }
            }
            remaining -= chunk;
            data += chunk;
        } else {
            if (arg3 != 0) {
                s32 block = block_map[0][i];
                s32 cmd_result;
                if (remaining == 0) {
                    cmd_result = 0;
                } else {
                    cmd_result =
                        queueVerifyBlockCommand(arg0, block, blocks_before + i,
                                                current_seq, data, remaining);
                }
                if (cmd_result < 0) {
                    cancelQueuedCardCommands(entries);
                    return cmd_result;
                }
            } else {
                result = fn_803ACC0C(arg0, block_map[0][i], blocks_before + i,
                                     current_seq, data, remaining);
                if (result < 0) {
                    retryCardClose(&arg0->file_info);
                    return result;
                }
                if (result > 0) {
                    goto writes_done;
                }
            }
            remaining = 0;
        }
    }

    if (arg3 != 0) {
        s32 cmd_result;
        cmd_result = queueFinishCardCommands(arg0);
        if (cmd_result < 0) {
            cancelQueuedCardCommands(entries);
            return cmd_result;
        }
    } else {
        result = retryCardClose(&arg0->file_info);
        if (result < 0) {
            return -267;
        }
        return 1;
    }
writes_done:
    while (secondary_count > 0) {
        secondary_count--;
        if (arg3 != 0) {
            s32 block = block_map[1][secondary_count];
            s32 cmd_result = queueDeleteBlockCommand(arg0, block, arg1);
            if (cmd_result < 0) {
                cancelQueuedCardCommands(entries);
                return cmd_result;
            }
        } else {
            result = fn_803ACFC0(arg0, block_map[1][secondary_count], 0xFFFF,
                                 0, NULL, 0, arg1);
            arg0->x170[block_map[1][secondary_count]] = -0x7FFF;
            arg0->x270[block_map[1][secondary_count]] = 0;
            if (result < 0) {
                retryCardClose(&arg0->file_info);
            }
            return result;
        }
    }

    current_seq = (u8) (current_seq + 1);
    data = arg2;
    remaining = file_size;
    for (i = 0; i < file_blocks && remaining > 0; i++) {
        s32 logical = blocks_before + i;
        if (logical == 0) {
            chunk = (arg0->x8 - 0x20) - ((arg0->x24 + 0x30) % arg0->x8);
        } else {
            chunk = arg0->x8 - 0x20;
        }

        if (remaining > chunk) {
            if (arg3 != 0) {
                s32 block = block_map[0][i];
                s32 cmd_result = queueWriteDataBlock(
                    arg0, block, logical, arg1, current_seq, data, chunk);
                if (cmd_result < 0) {
                    cancelQueuedCardCommands(entries);
                    return cmd_result;
                }
            } else {
                {
                    s32 block = block_map[0][i];
                    result = fn_803ACFC0(arg0, block, blocks_before + i,
                                         current_seq, data, chunk, arg1);
                }
                if (result < 0) {
                    arg0->x170[block_map[0][i]] = -0x7FFF;
                    arg0->x270[block_map[0][i]] = 0;
                    retryCardClose(&arg0->file_info);
                    return result;
                }
                arg0->x170[block_map[0][i]] = blocks_before + i;
                arg0->x270[block_map[0][i]] = current_seq;
            }
            remaining -= chunk;
            data += chunk;
        } else {
            if (arg3 != 0) {
                s32 block = block_map[0][i];
                s32 cmd_result = queueWriteRemainingDataBlock(
                    arg0, block, logical, arg1, current_seq, data, remaining);
                if (cmd_result < 0) {
                    cancelQueuedCardCommands(entries);
                    return cmd_result;
                }
            } else {
                result = fn_803ACFC0(arg0, block_map[0][i], blocks_before + i,
                                     current_seq, data, remaining, arg1);
                if (result < 0) {
                    arg0->x170[block_map[0][i]] = -0x7FFF;
                    arg0->x270[block_map[0][i]] = 0;
                    retryCardClose(&arg0->file_info);
                    return result;
                }
                arg0->x170[block_map[0][i]] = blocks_before + i;
                arg0->x270[block_map[0][i]] = current_seq;
            }
            remaining = 0;
        }
    }

    if (arg3 == 0) {
        result = retryCardClose(&arg0->file_info);
        if (result < 0) {
            return -267;
        }
    } else {
        entries[0].x0 = 4;
        entries[0].x4 = (s32) arg0;
        entries[0].x8 = (s32) arg4;
        entries[0].xC = arg1;
        hsd_804D7998 = -1;
    }

    return 0;
}

static inline s32 queueReadIcons(CardState* state, s32 file_id, s32 seq_num)
{
    s32 cmd[9];
    u32 block;
    s32 result;

    for (block = 0; block < (0x2F + state->x24 + state->x8) / state->x8;
         block++)
    {
        cmd[0] = 10;
        cmd[1] = (s32) state;
        cmd[3] = file_id;
        cmd[4] = seq_num;
        cmd[2] = block;
        result = fn_803AC168(cmd);
        if (result < 0) {
            return result;
        }
    }
    return 0;
}

static inline s32 queueLastIconCommand(s32* clear_cmd, s32* patch_cmd,
                                       CardState* state, s32 mode,
                                       u32 sector_size, s32 block)
{
    if (mode != 0) {
        clear_cmd[6] = (s32) state->x0;
        clear_cmd[0] = 4;
        clear_cmd[4] = 0;
        clear_cmd[5] = 0;
        clear_cmd[1] = (s32) state;
        clear_cmd[8] = sector_size;
        clear_cmd[7] = 0;
        clear_cmd[2] = 0;
        return fn_803AC168(clear_cmd);
    } else {
        s32 has_blocks;

        if (state->x4C[0] > 0) {
            has_blocks = 1;
        } else {
            has_blocks = 0;
        }
        patch_cmd[0] = 2;
        patch_cmd[1] = (s32) state;
        patch_cmd[4] = -1;
        patch_cmd[5] = 0;
        patch_cmd[6] = 0;
        patch_cmd[8] = has_blocks;
        patch_cmd[7] = block * sector_size;
        return fn_803AC168(patch_cmd);
    }
}

s32 fn_803B0E9C(struct CardState* arg0, s32 arg1, s32 arg2, s32 arg3, s32 arg4)
{
    u8 digest[0x30];
    s32 cmd_clear[9];
    s32 cmd_patch[9];
    s32 cmd_write_icon[9];
    s32 payload_pos;
    s32 remaining;
    s32 block_idx;
    s32 digest_idx;
    s32 result;
    u32 sector_size;

    arg0->x24 = hsd_803AC340(&arg0->x3B0);

    if (arg3 == 0) {
        if (arg4 != 0) {
            result = queueReadIcons(arg0, arg1, arg2);
            if (result < 0) {
                return result;
            }

            {
                s32 cmd_done[9];

                cmd_done[0] = 6;
                cmd_done[1] = (s32) arg0;
                result = fn_803AC168(cmd_done);
            }
            if (result < 0) {
                return result;
            }
        } else {
            result = fn_803ACD58(arg0, (UNK_T) arg1, (UNK_T) arg2);
            if (result < 0) {
                return result;
            }
            if (result == 0) {
                return 1;
            }
        }
    }

    if (arg4 != 0) {
        for (arg4 = 0; sector_size = arg0->x8,
            (u32) arg4 < (0x2F + arg0->x24 + sector_size) / sector_size;
             arg4++)
        {
            if ((u32) (arg4 + 1) ==
                (0x2F + arg0->x24 + sector_size) / sector_size)
            {
                result = queueLastIconCommand(cmd_clear, cmd_patch, arg0, arg3,
                                              sector_size, arg4);
                if (result < 0) {
                    return result;
                }
            }

            {
                cmd_write_icon[0] = 9;
                cmd_write_icon[1] = (s32) arg0;
                cmd_write_icon[2] = arg4;
                cmd_write_icon[3] = arg1;
                cmd_write_icon[4] = arg2;
                result = fn_803AC168(cmd_write_icon);
                if (result < 0) {
                    return result;
                }
            }
        }
        return 0;
    }

    sector_size = arg0->x8;
    if (arg0->x24 + 0x30 < sector_size) {
        if (arg3 != 0) {
            memset(arg0->x0, 0, sector_size);
        } else {
            result = retryCardRead(&arg0->file_info, arg0->x0, arg0->x8, 0);

            if (result < 0) {
                return result;
            }
        }
    }

    memcpy(arg0->x0, arg0->x370, 0x40);
    payload_pos = 0x40;
    switch (arg0->x3B0) {
    case 2:
        memcpy(arg0->x0 + 0x40, (UNK_T) arg1, 0x1800);
        payload_pos = 0x1840;
        break;
    case 1:
        memcpy(arg0->x0 + 0x40, (UNK_T) arg1, 0xE00);
        payload_pos = 0xE40;
        break;
    }

    memset(digest, 0, sizeof(digest));
    block_idx = 0;
    digest_idx = 0;
    remaining = arg0->x24 - payload_pos;

    while (remaining >= 0) {
        sector_size = arg0->x8;
        if ((u32) (payload_pos + remaining) > sector_size) {
            memcpy(arg0->x0 + payload_pos, (s32*) arg2,
                   sector_size - payload_pos);
            sector_size = arg0->x8;
            arg2 += sector_size - payload_pos;
            remaining -= sector_size - payload_pos;
            hsd_803B2B20(arg0->x0, sector_size, &digest[digest_idx]);

            result = retryCardWrite(&arg0->file_info, arg0->x0, arg0->x8,
                                    arg0->x8 * block_idx);

            if (result < 0) {
                return result;
            }

            payload_pos = 0;
            block_idx++;
            digest_idx += 0x10;

            if (remaining + 0x30 < (s32) arg0->x24) {
                if (arg3 != 0) {
                    memset(arg0->x0, 0, arg0->x8);
                } else {
                    result = retryCardRead(&arg0->file_info, arg0->x0,
                                           arg0->x8, arg0->x8 * block_idx);

                    if (result < 0) {
                        return result;
                    }
                }
            }
            continue;
        }

        memcpy(arg0->x0 + payload_pos, (void*) arg2, remaining);
        payload_pos += remaining;
        hsd_803B2B20(arg0->x0, payload_pos, &digest[digest_idx]);
        memcpy(arg0->x0 + payload_pos, digest, 0x30);
        remaining = -1;

        result = retryCardWrite(&arg0->file_info, arg0->x0, arg0->x8,
                                arg0->x8 * block_idx);

        if (result < 0) {
            return result;
        }

        block_idx++;
        digest_idx += 0x10;
    }

    return 0;
}

static inline s32 queueCardBlock(CardState* state, s32 logical, s32 phys,
                                 s32 file_id, void* data, s32 length,
                                 s32 offset)
{
    s32 write_cmd[9];

    if (logical == 0) {
        s32 seek_cmd[9];

        if (phys != 0) {
            return -0x101;
        }
        seek_cmd[0] = 2;
        seek_cmd[1] = (s32) state;
        seek_cmd[4] = 0;
        seek_cmd[5] = 0;
        seek_cmd[6] = 0;
        seek_cmd[8] = 0;
        seek_cmd[7] = offset;
        fn_803AC168(seek_cmd);
    }
    write_cmd[0] = 1;
    write_cmd[1] = (s32) state;
    write_cmd[3] = logical;
    write_cmd[4] = phys;
    write_cmd[5] = 0;
    write_cmd[6] = (s32) data;
    write_cmd[8] = length;
    write_cmd[7] = offset;
    write_cmd[2] = file_id;
    return fn_803AC168(write_cmd);
}

s32 fn_803B1338(CardState* state, s32 arg1)
{
    s32 result;
    s32 offset;
    s32 logical;
    s32 max_redun;
    s32 file_blocks;
    s32 file_id;
    s32 i;
    s32 phys;
    u8* fdata;

    max_redun = 0;
    phys = 1;
    logical = 1;

    if ((s32) fn_803AC634(state, 0) != 0) {
        s32 hdr_offset;
        u32 hdr_base;
        u32 sector_size;
        file_blocks = fn_803AC634(state, 0);
        offset = 0;
        sector_size = state->x8;
        hdr_base = state->x24 + 0x30;
        hdr_offset = hdr_base % sector_size;
        if (arg1 != 0) {
            fdata = state->x70[0].ptr;
            if (fdata == NULL) {
                s32 ofs0 = sector_size *
                           ((hdr_base + sector_size - 1) / sector_size - 1);
                result = queueCardBlock(state, 0, 0, 0, NULL, 0, ofs0);
            } else {
                s32 ofs0 = sector_size *
                           ((hdr_base + sector_size - 1) / sector_size - 1);
                result = queueCardBlock(state, 0, 0, 0, fdata,
                                        sector_size - hdr_offset - 0x20, ofs0);
                offset = state->x8 - hdr_offset - 0x20;
            }
            if (result < 0) {
                return result;
            }
        } else {
            fdata = state->x70[0].ptr;
            if (fdata == NULL) {
                result = fn_803ACFC0(state, 0, 0, 0, NULL, 0, 0);
            } else {
                result = fn_803ACFC0(state, 0, 0, 0, fdata,
                                     sector_size - hdr_offset - 0x20, 0);
                offset = state->x8 - hdr_offset - 0x20;
            }
            if (result < 0 && result != -0x105) {
                state->x170[0] = -0x7FFF;
                state->x270[0] = 0;
                return result;
            }
        }

        for (i = 1; i < file_blocks; i++) {
            if (arg1 != 0) {
                fdata = state->x70[0].ptr;
                if (fdata == NULL) {
                    s32 ofs = fn_803ACBE8(state, logical);
                    result =
                        queueCardBlock(state, logical, phys, 0, NULL, 0, ofs);
                } else {
                    u8* wdata = fdata + offset;
                    s32 chunk = state->x8 - 0x20;
                    s32 ofs = fn_803ACBE8(state, logical);
                    result = queueCardBlock(state, logical, phys, 0, wdata,
                                            chunk, ofs);
                    offset = state->x8 + offset;
                    offset -= 0x20;
                }
                if (result < 0) {
                    return result;
                }
            } else {
                fdata = state->x70[0].ptr;
                if (fdata == NULL) {
                    result =
                        CARD_WRITE_BLOCK(state, logical, phys, 0, NULL, 0, 0);
                } else {
                    result =
                        CARD_WRITE_BLOCK(state, logical, phys, 0,
                                         fdata + offset, state->x8 - 0x20, 0);
                    offset = state->x8 + offset;
                    offset -= 0x20;
                }
                if (result < 0) {
                    state->x170[logical] = -0x7FFF;
                    state->x270[logical] = 0;
                    return result;
                }
                state->x170[logical] = phys;
                state->x270[logical] = 0;
            }
            logical++;
            phys++;
        }
    }

    for (file_id = 1; file_id < 9; file_id++) {
        s32 j;
        file_blocks = fn_803AC634(state, file_id);
        offset = 0;
        if (file_blocks > 0) {
            for (j = 0; j < file_blocks; j++) {
                if (arg1 != 0) {
                    fdata = state->x70[file_id].ptr;
                    if (fdata == NULL) {
                        s32 ofs = fn_803ACBE8(state, logical);
                        result = queueCardBlock(state, logical, phys, file_id,
                                                NULL, 0, ofs);
                    } else {
                        u8* wdata = fdata + offset;
                        s32 chunk = state->x8 - 0x20;
                        s32 ofs = fn_803ACBE8(state, logical);
                        result = queueCardBlock(state, logical, phys, file_id,
                                                wdata, chunk, ofs);
                        offset = state->x8 + offset;
                        offset -= 0x20;
                    }
                    if (result < 0) {
                        return result;
                    }
                } else {
                    fdata = state->x70[file_id].ptr;
                    if (fdata == NULL) {
                        result = fn_803ACFC0(state, logical, phys, 0, NULL, 0,
                                             file_id);
                    } else {
                        result = fn_803ACFC0(state, logical, phys, 0,
                                             fdata + offset, state->x8 - 0x20,
                                             file_id);
                        offset = state->x8 + offset;
                        offset -= 0x20;
                    }
                    if (result < 0) {
                        state->x170[logical] = -0x7FFF;
                        state->x270[logical] = 0;
                        return result;
                    }
                    state->x170[logical] = phys;
                    state->x270[logical] = 0;
                }
                logical++;
                phys++;
            }
            switch (state->x28[file_id]) {
            case 0:
                break;
            case 1:
                if (max_redun < file_blocks) {
                    max_redun = file_blocks;
                }
                break;
            case 2:
                if (max_redun < 1) {
                    max_redun = 1;
                }
                break;
            case 3:
                break;
            }
        }
    }

    for (i = 0; i < max_redun; i++) {
        if (arg1 != 0) {
            s32 ofs = fn_803ACBE8(state, logical);
            if (logical == 0) {
                result = -257;
            } else {
                result = queueCardCommand1(state, logical, 0xFFFF, file_id,
                                           NULL, 0, ofs);
            }
            if (result < 0) {
                return result;
            }
        } else {
            result = fn_803ACFC0(state, logical, 0xFFFF, 0, NULL, 0, file_id);
            state->x170[logical] = -0x7FFF;
            state->x270[logical] = 0;
            if (result < 0) {
                return result;
            }
        }
        phys++;
        logical++;
    }

    phys = 1;
    if ((s32) fn_803AC634(state, 0) != 0) {
        phys = fn_803AC634(state, 0);
    }
    for (file_id = 1; file_id < 9; file_id++) {
        s32 k;
        file_blocks = fn_803AC634(state, file_id);
        offset = 0;
        if (file_blocks > 0) {
            switch (state->x28[file_id]) {
            case 0:
                for (k = 0; k < file_blocks; k++) {
                    if (arg1 != 0) {
                        fdata = state->x70[file_id].ptr;
                        if (fdata == NULL) {
                            s32 ofs = fn_803ACBE8(state, logical);
                            result = queueCardBlock(state, logical, phys,
                                                    file_id, NULL, 0, ofs);
                        } else {
                            u8* wdata = fdata + offset;
                            s32 chunk = state->x8 - 0x20;
                            s32 ofs = fn_803ACBE8(state, logical);
                            result =
                                queueCardBlock(state, logical, phys, file_id,
                                               wdata, chunk, ofs);
                            offset = state->x8 + offset;
                            offset -= 0x20;
                        }
                        if (result < 0) {
                            return result;
                        }
                    } else {
                        fdata = state->x70[file_id].ptr;
                        if (fdata == NULL) {
                            s32 write_result = fn_803ACFC0(
                                state, logical, phys, 0, NULL, 0, file_id);
                            result = write_result;
                        } else {
                            s32 write_result = fn_803ACFC0(
                                state, logical, phys, 0, fdata + offset,
                                state->x8 - 0x20, file_id);
                            result = write_result;
                            offset = state->x8 + offset;
                            offset -= 0x20;
                        }
                        if (result < 0) {
                            state->x170[logical] = -0x7FFF;
                            state->x270[logical] = 0;
                            return result;
                        }
                        state->x170[logical] = phys;
                        state->x270[logical] = 0;
                    }
                    logical++;
                    phys++;
                }
                break;
            case 1:
            case 2:
            case 3:
                phys += file_blocks;
                break;
            }
        }
    }

    return 0;
}

s32 fn_803B1F78(CardState* state, s32 channel, s32 file_id, s32 seq_num,
                s32 callback)
{
    s32 buf1[9];
    s32 buf2[9];
    s32 result;
    s32 blocks;
    s32 saved;
    s32 snap;
    s32 saved1;
    s32 snap1;
    s32 saved2;
    s32 snap2;
    s32 saved3;
    s32 snap3;
    s32 saved4;
    s32 snap4;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    PAD_STACK(12);

    hsd_804D7998 = hsd_804D7984;
    blocks = state->x8 * hsd_803B2674(state);
    buf1[0] = 7;
    buf1[1] = (s32) state;
    buf1[2] = channel;
    buf1[3] = blocks;
    result = fn_803AC168(buf1);
    if (result < 0) {
        snap1 = hsd_804D7998;
        if (snap1 >= 0) {
            saved1 = snap1;
            while (saved1 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved1].x10 = 0;
                saved1 = (saved1 + 1) % 128;
            }
            hsd_804D7984 = snap1;
        }
        return result;
    }

    result = fn_803B0E9C(state, file_id, seq_num, 1, 1);
    if (result < 0) {
        snap2 = hsd_804D7998;
        if (snap2 >= 0) {
            saved2 = snap2;
            while (saved2 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved2].x10 = 0;
                saved2 = (saved2 + 1) % 128;
            }
            hsd_804D7984 = snap2;
        }
        return result;
    }

    result = fn_803B1338(state, 1);
    if (result < 0) {
        snap3 = hsd_804D7998;
        if (snap3 >= 0) {
            saved3 = snap3;
            while (saved3 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved3].x10 = 0;
                saved3 = (saved3 + 1) % 128;
            }
            hsd_804D7984 = snap3;
        }
        return result;
    }

    buf2[0] = 8;
    buf2[1] = (s32) state;
    result = fn_803AC168(buf2);
    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            saved = snap;
            while (saved != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved].x10 = 0;
                saved = (saved + 1) % 128;
            }
            hsd_804D7984 = snap;
        }
        return result;
    }

    entries[0].x0 = 6;
    entries[0].x4 = (s32) state;
    entries[0].x8 = callback;
    result = 0;
    entries[0].xC = result;
    if (result < 0) {
        snap4 = hsd_804D7998;
        if (snap4 >= 0) {
            saved4 = snap4;
            while (saved4 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved4].x10 = 0;
                saved4 = (saved4 + 1) % 128;
            }
            hsd_804D7984 = snap4;
        }
        return 0;
    }
    hsd_804D7998 = -1;
    return 0;
}

s32 fn_803B21E8(s32 card_state, s32 file_id, s32 seq_num, s32 callback)
{
    s32 buf[9];
    s32 result;
    s32 saved;
    s32 snap;
    s32 saved1;
    s32 snap1;
    s32 saved2;
    s32 snap2;
    CardBufEntry* entries = (CardBufEntry*) hsd_804D1138;
    PAD_STACK(8);

    hsd_804D7998 = hsd_804D7984;
    result = fn_803B0E9C((CardState*) card_state, file_id, seq_num, 0, 1);
    if (result < 0) {
        snap1 = hsd_804D7998;
        if (snap1 >= 0) {
            saved1 = snap1;
            while (saved1 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved1].x10 = 0;
                saved1 = (saved1 + 1) % 128;
            }
            hsd_804D7984 = snap1;
        }
        return result;
    }

    buf[0] = 8;
    buf[1] = card_state;
    result = fn_803AC168(buf);
    if (result < 0) {
        snap2 = hsd_804D7998;
        if (snap2 >= 0) {
            saved2 = snap2;
            while (saved2 != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved2].x10 = 0;
                saved2 = (saved2 + 1) % 128;
            }
            hsd_804D7984 = snap2;
        }
        return result;
    }

    entries[0].x0 = 7;
    entries[0].x4 = card_state;
    entries[0].x8 = callback;
    result = 0;
    entries[0].xC = result;
    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            saved = snap;
            while (saved != hsd_804D7984) {
                ((CardBufEntry*) hsd_804D1138)[saved].x10 = 0;
                saved = (saved + 1) % 128;
            }
            hsd_804D7984 = snap;
        }
        return 0;
    }
    hsd_804D7998 = -1;
    return 0;
}

void hsd_803B2374(void)
{
    int i;

    hsd_804D7990 = 0;
    hsd_804D7994 = 0;
    memset(&hsd_804D2348, 0, sizeof(hsd_804D2348));
    hsd_804D7980 = 0;
    hsd_804D7984 = 0;
    hsd_804D799C = 2;
    for (i = 0; i < 128; i++) {
        hsd_804D1148[i][0] = 0;
    }
    hsd_804D7988 = 0;
}

void hsd_803B24E4(s32* ctx, int channel, int file_no, void* work_buf)
{
    memset(ctx, 0, 0x464);
    ctx[8] = -1;
    ctx[1] = channel;
    ctx[2] = file_no;
    ctx[0] = (s32) work_buf;
}

inline HsdCmdEntry* hsd_803B2550_inline(u8* arg0, s32 arg1)
{
    return &((HsdCmdEntry*) (arg0 + 0x1210))[arg1];
}

int hsd_803B2550(s32* arg0, const char* arg1, void (*arg2)(int, int))
{
    s32 new_var;
    u8* base = hsd_804D1138;
    s32 chan = arg0[1];
    s32 new_var3;
    s32 new_var2;
    s32 retries;
    s32 result;
    s32 write_idx;
    new_var2 = chan;
    for (retries = 0; retries < 10; retries++) {
        result = CARDOpen(new_var2, (char*) arg1, (CARDFileInfo*) (arg0 + 3));
        if (result != -1) {
            break;
        }
    }

    if (result < 0) {
        return result;
    }

    result = 0;
    {
        s32 tmp = arg0[4];
        do {
            if (tmp != -1) {
                break;
            }
            result++;
        } while (result < 10);
        write_idx = arg0[4];
        retries = (new_var = (new_var3 = arg0[4]));
        write_idx = retries;
        if (tmp < 0) {
            return new_var;
        }
    }

    for (chan = 0; chan < 10; chan++) {
        if (CARDClose((CARDFileInfo*) (arg0 + 3)) != -1) {
            break;
        }
    }

    {
        s32 read_idx = hsd_804D7990;
        retries = hsd_804D7994;

        if (read_idx == retries) {
            if (CMD_QUEUE(base)[read_idx].type != 0) {
                return -265;
            }
        }
    }

    {
        HsdCmdEntry* entry = hsd_803B2550_inline(base, retries);
        s32 next = retries + 1;
        entry->type = 5;
        entry->f1 = (s32) arg0;
        entry->f2 = write_idx;
        entry->f5 = (s32) arg2;
        hsd_804D7994 = next % 32;
    }

    return 0;
}

s32 hsd_803B2674(CardState* state)
{
    s32 blocks;

    state->x24 = hsd_803AC340(&state->x3B0);

    blocks = (0x2F + state->x24 + state->x8) / state->x8;
    blocks += fn_803AC7DC(state);

    return blocks;
}

static inline void initHeaderBlockCommand(s32* buf, CardState* state,
                                          u32 block, s32 file_id, s32 seq_num,
                                          s32 version)
{
    buf[0] = 11;
    buf[1] = (s32) state;
    buf[2] = block;
    buf[3] = file_id;
    buf[4] = seq_num;
    buf[5] = version;
}

static inline s32 queueHeaderBlock(CardState* state, u32 block, s32 file_id,
                                   s32 seq_num, s32 version)
{
    s32 buf[9];

    initHeaderBlockCommand(buf, state, block, file_id, seq_num, version);
    return fn_803AC168(buf);
}

static inline s32 queueHeaderBlocks(CardState* state, s32 file_id, s32 seq_num,
                                    s32 version)
{
    u32 i;
    int result;

    for (i = 0; i < (0x2F + state->x24 + state->x8) / state->x8; i++) {
        result = queueHeaderBlock(state, i, file_id, seq_num, version);
        if (result < 0) {
            return result;
        }
    }
    return 0;
}

s32 fn_803B26CC(CardState* state, s32 file_id, s32 seq_num, s32 version,
                void (*callback)(s32, s32))
{
    s32 result;
    s32 snap;
    CardContext* context = (CardContext*) hsd_804D1138;

    state->x24 = hsd_803AC340(&state->x3B0);
    hsd_804D7998 = hsd_804D7984;

    result = queueHeaderBlocks(state, file_id, seq_num, version);

    if (result < 0) {
        snap = hsd_804D7998;
        if (snap >= 0) {
            rollbackCardCommands(context, snap);
        }
        return result;
    }

    context->x0 = 5;
    context->x4 = state;
    context->x8 = callback;
    context->xC = 0;
    hsd_804D7998 = -1;
    return 0;
}
